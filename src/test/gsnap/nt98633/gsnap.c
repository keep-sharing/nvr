...
//	row_stride = fb_width(fb) * 2;
	...
	while (cinfo.next_scanline < cinfo.image_height) 
	{
		int i = 0;
		int offset = 0;
		unsigned char* line = fb->bits + fb_stride(fb)* (fb->vi.yoffset + cinfo.next_scanline);

		
		for(i = 0; i < fb_width(fb); i++, offset += 3, line += fb_bpp(fb))
		{
			fb->unpack(fb, line, (unsigned char*)image_buffer+offset, (unsigned char*)image_buffer + offset + 1, (unsigned char*)image_buffer + offset + 2);
		}
...
static int snap2png(const char * filename, int quality, FBInfo* fb)
{
	...
	for( ; i < height; i++ )
	{
		unsigned char* line = fb->bits + fb_stride(fb)* (fb->vi.yoffset + i);
