...
static int register_mod(struct module *mod)
{
    int ret = 0;
    
    if (!strcmp(mod->name, "mod_cpu")) {
        mod_cpu_register(mod);
    } else if (!strcmp(mod->name, "mod_udp")) {
        mod_udp_register(mod);
    } else if (!strcmp(mod->name, "mod_pcsw")) {
        mod_pcsw_register(mod);
    } else if (!strcmp(mod->name, "mod_partition")) {
        mod_partition_register(mod);
    } else if (!strcmp(mod->name, "mod_nginx")) {
        mod_nginx_register(mod);
    } else if (!strcmp(mod->name, "mod_ncpu")) {
        mod_ncpu_register(mod);
    } else if (!strcmp(mod->name, "mod_mem")) {
        mod_mem_register(mod);
    } else if (!strcmp(mod->name, "mod_lvs")) {
        mod_lvs_register(mod);
    } else if (!strcmp(mod->name, "mod_load")) {
        mod_load_register(mod);
    } else if (!strcmp(mod->name, "mod_io")) {
        mod_io_register(mod);
    } else if (!strcmp(mod->name, "mod_haproxy")) {
        mod_haproxy_register(mod);
    } else if (!strcmp(mod->name, "mod_apache")) {
        mod_apache_register(mod);
    } else if (!strcmp(mod->name, "mod_percpu")) {
        mod_percpu_register(mod);
    } else if (!strcmp(mod->name, "mod_pernic")) {
        mod_pernic_register(mod);
    } else if (!strcmp(mod->name, "mod_proc")) {
        mod_proc_register(mod);
    } else if (!strcmp(mod->name, "mod_squid")) {
        mod_squid_register(mod);
    } else if (!strcmp(mod->name, "mod_swap")) {
        mod_swap_register(mod);
    } else if (!strcmp(mod->name, "mod_tcp")) {
        mod_tcp_register(mod);
    } else if (!strcmp(mod->name, "mod_tcpx")) {
        mod_tcpx_register(mod);
    } else if (!strcmp(mod->name, "mod_traffic")) {
        mod_traffic_register(mod);
    } else {
        ret = -1;
    }
...
load_modules()
{
    ...
    for (i = 0; i < statis.total_mod_num; i++) {
        ...
        if (!register_mod(mod)) {
            ...
            do_debug(LOG_ERR, "load_modules: load module %s failed.\n", mod->name);
        }
    }
}
            ...
            if (strpbrk(mod->record, ITEM_SPLIT)) {
                ...
                if (MERGE_ITEM == conf.print_merge) {
                    ...
                    while ((item = strtok_next_item(mod->record, &pos)) != NULL) {
                        if (strFlag) {
                            if ((ret=convert_record_to_array_double(&mod->st_array[num * mod->n_col], mod->n_col, item)) <= 0) {
                                break;
                            }
                        } else {
                            if ((ret=convert_record_to_array_ull(&mod->cur_array[num * mod->n_col], mod->n_col, item)) <= 0) {
                                break;
                            }
                        }
                        num++;
                ...
                if (strFlag) {
                    ret = convert_record_to_array_double(mod->st_array, mod->n_col, mod->record);
                } else {
                    ret = convert_record_to_array_ull(mod->cur_array, mod->n_col, mod->record);
                }
            }
...
{
    if (!src || !key || key[0] == 0 || !dst || len <= 0) {
        return -1;
    }
    const char *pFind = strstr(src, key);
    if (!pFind) {
        return -1;
    }
    pFind += strlen(key);
