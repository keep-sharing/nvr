...
print_header()
{
    ...
    sprintf(opt_line, "%s%s", TITLE_STRING, PRINT_SEC_SPLIT);
    sprintf(hdr_line, "%s%s", HEADER_STRING, PRINT_SEC_SPLIT);
...
running_print_live()
{
    ...
    while (1) {
        ...
        if ((DEFAULT_PRINT_NUM > 0 && !((print_num) % DEFAULT_PRINT_NUM)) || re_p_hdr) {
...
static long trans_date_to_sec(char *pTime)
{
	if(pTime[0] == '\0') return 0;
	struct tm m_tm = {0};
	m_tm.tm_isdst = -1;
	long ntime = 0;
	strptime(pTime, "%Y-%m-%d %H:%M:%S", &m_tm);
	ntime = mktime(&m_tm);
...
static int get_record_time(const char *line, char *sTime)
{
    char *token;
...
void
print_record_time(char *sTime)
{
    printf("%s%s", sTime, PRINT_SEC_SPLIT);
}
...
print_tail(int tail_type)
{
    ...
    switch (tail_type) {
        case TAIL_MAX:
            printf("%s%s", MAX_STRING, PRINT_SEC_SPLIT);
            break;
        case TAIL_MEAN:
            printf("%s%s", MEAN_STRING, PRINT_SEC_SPLIT);
            break;
        case TAIL_MIN:
            printf("%s%s", MIN_STRING, PRINT_SEC_SPLIT);
            break;
        default:
            return;
    }
...
static int tsar_parse_config_paths(char *pattern, char *dirPath, int dirLen, char *fileName, int fileLen)
{
    char *p, *tmp;
    int ret = -1;
    int cnt = 0;
    ...
    //printf("patten:%s ## dir:%s ## filename:%s\n", pattern, dirPath, fileName);
    return ret;
}

static int tsar_filter(const struct dirent *pDir)
{
    if (strstr(pDir->d_name, conf.print_file_filter)) {
        return 1;
    }
...
static int tsar_compar(const struct dirent **a, const struct dirent **b)
{
    return alphasort(a, b);
}

FILE *
init_running_print_ex()
{
    FILE *fp;
    ...
    if (tsar_parse_config_paths(conf.output_file_path, conf.print_file_dir, sizeof(conf.print_file_dir),
                                conf.print_file_file, sizeof(conf.print_file_file)) < 0) {
        do_debug(LOG_FATAL, "file path %s invalid.\n", conf.output_file_path);
    }
    snprintf(conf.print_file_filter, sizeof(conf.print_file_filter), "%s%s", conf.print_file_file, ".");
    conf.print_file_number = 0;
    conf.print_file_index = 0;
    sum = scandir(conf.print_file_dir, &ents, tsar_filter, tsar_compar);
    if (sum < 0) {
        do_debug(LOG_FATAL, "not file exist.\n", conf.output_file_path);
    }

    if (sum != -1) {
        while (offset < sum) {
            if (conf.print_file_number < TASR_MAX_FILE_COUNT - 1) {
                snprintf(conf.print_file_name[conf.print_file_number], sizeof(conf.print_file_name[conf.print_file_number]),
                         "%s%s", conf.print_file_dir, ents[offset]->d_name);
                conf.print_file_number++;
            } else {
                printf("file count more than %d ! %s\n", TASR_MAX_FILE_COUNT, ents[offset]->d_name);
            }
            free(ents[offset]);
            offset++;
        }
    ...
    snprintf(conf.print_file_name[conf.print_file_number], sizeof(conf.print_file_name[conf.print_file_number]), "%s", conf.output_file_path);
    conf.print_file_number++;
    conf.print_tail = 1;
    fp = fopen(conf.print_file_name[conf.print_file_index], "r");
    if (!fp) {
        do_debug(LOG_FATAL, "unable to open the log file %s\n", conf.print_file_name[conf.print_file_index]);
    }
...
running_print()
{
    ...
    while (1) {
        if (!fgets(line, sizeof(line), fp)) {
            if (conf.print_file_index >= (conf.print_file_number-1)) {
                break;
            } else {
                conf.print_file_index++;
                if (fclose(fp) < 0) {
                    do_debug(LOG_FATAL, "fclose error:%s", strerror(errno));
                }
                //printf("num:%d index:%d name:%s\n", conf.print_file_number, conf.print_file_index, conf.print_file_name[conf.print_file_index]);
                fp = fopen(conf.print_file_name[conf.print_file_index], "r");
                if (!fp) {
                    do_debug(LOG_FATAL, "unable to open the log file %s.\n", conf.print_file_name[conf.print_file_index]);
                }
...
running_check(int check_type)
{
    ...
    if (0 != gethostname(host_name, sizeof(host_name))) {
        do_debug(LOG_ERR, "tsar -check: gethostname err, errno=%d\n", errno);
        return;
    }
    i = 0;
    while (host_name[i]) {
    ...
    fp = fopen(filename, "r");
    if (!fp) {
        do_debug(LOG_ERR, "unable to open the log file %s.\n", filename);
        return;
    }
    ...
    if (nowtime - statbuf.st_mtime > 300) {
        do_debug(LOG_WARN, "/var/log/tsar.data is far away from now, now time is %d, last time is %d\n", nowtime, statbuf.st_mtime);
        fclose(fp);
        return;
    }
    /*FIX ME*/
    ...
    if (total_num == 0) {
        if (fclose(fp) < 0) {
            do_debug(LOG_ERR, "fclose error:%s\n", strerror(errno));
            return;
        }
        fp = NULL;
        ...
        total_num = seek_tail_lines(fp, 2, len);
        if (total_num < 2) {
            do_debug(LOG_ERR, "not enough lines at log file %s.\n", filename);
            fclose(fp);
            return;
        }
        ...
        if (!fgets(line[0], len[0] + 2, fp)) {
            do_debug(LOG_ERR, "fgets error:%s\n", strerror(errno));
            fclose(fp);
            return;
        }
    ...
    if (ts[0] && ts[1]) {
        ...
        if (conf.print_interval == 0) {
            do_debug(LOG_ERR, "running tsar -c too frequently\n");
            return;
        }
    }
    collect_record_stat();
