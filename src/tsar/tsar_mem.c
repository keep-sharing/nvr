...
read_mem_stats(struct module *mod)
{
    ...
    st_mem.usedkb = st_mem.tlmkb - st_mem.frmkb - st_mem.bufkb - st_mem.camkb;
    int pos = sprintf(buf, "%lu,%lu,%lu,%lu",
            st_mem.frmkb,
            st_mem.usedkb,
            st_mem.bufkb,
            st_mem.camkb);
    ...
    U_64 pre_array[], U_64 cur_array[], int inter)
{
    ...
    st_array[1] = cur_array[1]<<10;
    st_array[2] = cur_array[2]<<10;
    st_array[3] = cur_array[3]<<10;
    st_array[4] = st_array[0] + st_array[1] + st_array[2] + st_array[3];
    st_array[5] = st_array[1] * 100.0 / (st_array[4]);
}

static struct mod_info mem_info[] = {
    {"  free", SUMMARY_BIT,  0,  STATS_NULL},
...
void
mod_mem_register(struct module *mod)
{
