...
set_debug_level()
{
    ...
    if (token) {
        ...
        } else if (!strcasecmp(token, "FATAL")) {
            conf.debug_level = LOG_FATAL;
        } else {
            conf.debug_level = LOG_ERR;
        }

        snprintf(conf.debugLevelStr, sizeof(conf.debugLevelStr), "%s", token);
    }
}
...
parse_line(char *buff)
{
    ...
    } else if (!strcmp(token, "tsar_log_stdout")) {
        parse_string(conf.op_stdout);
