...
static int http_rawdata(struct http_parser_t *http, const void* data, size_t bytes)
{
	...
	{
        capacity = http->raw_size + bytes + 1 + 2*KB;
        if (capacity > 4*MB) {
            printf("capacity %d more than limit.\n", capacity);
            return -1;
        }
	...
	if(http->raw_capacity - http->raw_size <= bytes)
	{
		return -ENOMEM;
	}
...
static int http_header_handler(struct http_parser_t *http, size_t npos, size_t vpos)
{
	...
	if(0 == strcasecmp("Content-Length", name))
	{
		...
		if(http->content_length >= 0 && 0!=s_body_max_size && http->content_length > (int)s_body_max_size)
		{
			printf("content length is too big content_length:%d\n", http->content_length);
		}
	}
	...
	else if(0 == strcasecmp("Transfer-Encoding", name))
	{
		...
		if(0 == strncasecmp("chunked", value, 7))
		{
			...
			{
				printf("chunked mode have content_length :%d\n", http->content_length);
			}
...
static int http_header_add(struct http_parser_t *http, struct http_header_t* header)
{
	...
	if(header->name.pos == 0||header->name.len == 0|| header->value.pos == 0|| !is_valid_token(http->raw + header->name.pos, header->name.len))
	{
		return -1;
	}
...
static int http_parse_status_line(struct http_parser_t *http)
{
	...
	for(; http->offset < http->raw_size; http->offset++)
	{
		...
		case SM_STATUS_CR:
			if ('\n' != c)
			{
				//printf("lost CR, maybe packet error or lost packet\n");
				return -1;
			}
			http->stateM = SM_STATUS_LF;
			http->offset -= 1; // go back
		...
		case SM_STATUS_LF:
			if('\n' != c)
			{
				//printf("lost LF, maybe packet error or lost packet\n");
				return -1;
			}
			// H3.1 HTTP Version (p13)
			// HTTP-Version = "HTTP" "/" 1*DIGIT "." 1*DIGIT
			...
			{
				//printf("header len error, maybe packet error or lost packet\n");
				return -1;
			}
...
static int http_parse_chunked(struct http_parser_t *http)
{
	...
	if(0 == http->chunk.offset)
	{
		...
		if(-1 != http->content_length)
		{
			printf("chunked error http->content_length: %d\n", http->content_length);
		}
	...
	for(; http->chunk.offset < http->raw_size; http->chunk.offset++)
	{
		...
		case CHUNK_DATA:
			...
			if(http->chunk.len == 0 || http->chunk.pos == 0)
			{
				return -1;
			}
...
int http_parser_destroy(struct http_parser_t* http)
{
	if(http->raw)
	{
		if(http->raw_capacity == 0)
		{
			printf("http parser raw_capacity error, %u\n", http->raw_capacity);
		}
		free(http->raw);
		http->raw = 0;
		http->raw_size = 0;
	...
	if(http->headers)
	{
		if(http->header_capacity <= 0)
		{
			printf("http parser header_capacity error, %d\n", http->header_capacity);
		}
		free(http->headers);
		http->headers = 0;
		http->header_size = 0;
...
int http_parser_input(struct http_parser_t* http, const void* data, size_t *bytes)
{
    ...
    //printf("[david debug] stateM:%d 00\n", http->stateM);
	// save raw data
	r = http_rawdata(http, data, *bytes);
	if(0 != r)
	{
	    printf("===error http_rawdata r:%d capacity:%d used:%d bytes:%d===\n", r, http->raw_capacity, http->raw_size, *bytes);
		//assert(r < 0);
		return r;
	}
    //printf("[david debug] stateM:%d 11\n", http->stateM);
	...
	if(SM_BODY <= http->stateM && http->stateM < SM_DONE)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					printf("you got extra size, raw_size:%u,offset:%u,content_length:%d\n",http->raw_size, http->offset, http->content_length);
				}
				if(http->raw_size >= http->offset + http->content_length)
					http->stateM = SM_DONE;
                
                //printf("[david debug] raw_size:%d offset:%d content_length:%d 33-2\n", http->raw_size, http->offset, http->content_length);
                /*David modify for rtsp over http POST reqs 20210915*/
                if (strstr(data, "POST") && strstr(data, "/ms/httpstream")) {
                    if(http->raw_size == http->offset) {
                        http->stateM = SM_DONE;
                        *bytes = 0;
                        return 0;
                    }
    ...
    //printf("[david debug] stateM:%d 44\n", http->stateM);
	if(r < 0) {
		return r;
	}
	...
	*bytes = 0;
	if (SM_DONE == http->stateM)
	{
		...
		if(http->content_length >= 0 &&  http->raw_size - http->offset < (size_t)http->content_length)
		{
			//errror raw_size
			printf("error raw_size:%u,offset:%u,content_length:%d\n", http->raw_size, http->offset, http->content_length);
			return -1;
		}
		*bytes = http->raw_size - http->offset - ((http->content_length >= 0) ? http->content_length : 0);
	}
    //printf("[david debug] stateM:%d 55\n", http->stateM);
...
//for Multiple message-header fields with the same field-name MAY be present in a message
//
const char* http_get_header_by_name_and_ignorepos(const struct http_parser_t* http, const char* name, int ignore_pos)
{
	int i, count = 0;
	assert(http->stateM>=SM_BODY);
	...
	for(i = 0; i < http->header_size; i++)
	{
		...
		// RFC-2616 4.2 Message Headers p22
		// Multiple message-header fields with the same field-name MAY be present in a message
		if(0 == strcasecmp(http->raw + http->headers[i].name.pos, name) && count++ >= ignore_pos)
			return http->raw + http->headers[i].value.pos;
	}
	return NULL; // not found
}
