...
#else /*David Modify gmtime -> gmtime_r*/
const char* rfc822_datetime_format(time_t time, rfc822_datetime_t datetime)
{
	int r;
	struct tm utc_tm;
	...
	assert(0 <= utc_tm.tm_wday && utc_tm.tm_wday < 7);
	assert(0 <= utc_tm.tm_mon && utc_tm.tm_mon < 12);
	assert(sizeof(rfc822_datetime_t) >= 30);
	r = snprintf(datetime, sizeof(rfc822_datetime_t), "%s, %02d %s %04d %02d:%02d:%02d GMT",
		s_week[(unsigned int)utc_tm.tm_wday % 7],
		utc_tm.tm_mday,
		s_month[(unsigned int)utc_tm.tm_mon % 12],
		utc_tm.tm_year+1900,
		utc_tm.tm_hour,
		utc_tm.tm_min,
		utc_tm.tm_sec);
