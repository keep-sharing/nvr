...
#if 1 /*David Eliminate compilation warnings 20211109*/
...
#else
inline int socket_setreuseport(IN socket_t sock, IN int enable); // reuse port. 
inline int socket_getreuseport(IN socket_t sock, OUT int* enable);
#endif
...
#else
inline int socket_setreuseport(IN socket_t sock, IN int enable)
{
	return socket_setopt_bool(sock, SO_REUSEPORT, enable);
}
...
{
    if (!sa || !port) {
        printf("socket_addr_to invalid params. addr:%p port:%p\n", sa, port);
        return -1; 
    }
