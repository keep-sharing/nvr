...
void* rtp_payload_decode_create(int payload, const char* name, struct rtp_payload_t *handler, void* cbparam)//(ms_rtp_session_context_t *)cbparam
{
	struct rtp_payload_delegate_t* ctx;
	ctx = calloc(1, sizeof(*ctx));
	if (ctx)
...
static int rtp_payload_find(int payload, const char* encoding, struct rtp_payload_delegate_t* codec)
{
	...
	{
		printf("payload is out of range,%d,encodeing:%s\n", payload, encoding);
		return -1;
	}
	if ((payload >= 96 || payload == 35) && encoding)//35: bosch h264
	{
			...
			|| 0 == strcasecmp(encoding, "MP1S"))  // MPEG-1 Systems Streams video (RFC 2250)
		{
			...
			|| 0 == strcasecmp(encoding, "vnd.onvif.metadata")//for onvif metadata stream
			|| 0 == strcasecmp(encoding, "ALL-G726-16")//for G726-16
			|| 0 == strcasecmp(encoding, "ALL2-G726-16"))//for G726-16
		{
