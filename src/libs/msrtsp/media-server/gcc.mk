...
ifeq ($(OUTTYPE),1)
	$(CXX) -o $@ -shared -fPIC $(LDFLAGS) $^ $(addprefix -L,$(LIBPATHS)) $(addprefix -l,$(LIBS))
else
	$(AR) -rcs $@ $^
endif
endif
	@cp $(OUTPATH)/$(OUTFILE) $(INSTALL_DIR)
ifeq ($(OUTTYPE_INCLUDE),1)
	# make output dir
	$(shell mkdir -p $(HEADFILE_INSTALL_DIR)/$(OUTFILE_INCLUDE_PATH)/ > /dev/null)
	@cp $(INNER_INCLUDE_PATH) $(HEADFILE_INSTALL_DIR)/$(OUTFILE_INCLUDE_PATH)/
endif	
