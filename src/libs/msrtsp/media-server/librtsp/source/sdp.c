...
static int sdp_parse_origin(struct sdp_t* sdp)
{
    ...
    /*Compatible Honeywel IPC 2021.08.27*/
    num = sdp_token_crlf(sdp);
    if (num) { 
        sdp_skip_space(sdp);
        num = sdp_token_word(sdp, " \t\r\n");
    }
    
    if (num) {
        if(sdp_token_crlf(sdp)) {
            printf("===[david debug] sdp_token_crlf is err!!!===\n");
            return -1;
        }
    }
...
static int sdp_parse_attribute(struct sdp_t* sdp)
{
	...
	if(0 == sdp_token_crlf(sdp))
	{
		...
		a->name[n[0]] = '\0';
		a->value[n[1]] = '\0';
		//printf("[daivd debug] name:%s value:%s\n", a->name, a->value);
		return 0;
	}
	return -1;
}
