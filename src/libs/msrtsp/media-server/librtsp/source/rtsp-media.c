	...
	if (name)
	{
		...
		else if (0 == strcmp("fmtp", name))
		{
			...
			for (i = 0; i < media->avformat_count && media->offset + n + 1 < sizeof(media->ptr); i++)
			{
				...
				media->offset += n + 1;
				//printf("[david debug] i:%d encoding:%s\n", i, media->avformats[i].encoding);
				if(0 == strcmp("H264", media->avformats[i].encoding))
				{
					...
					if(h264.flags & SDP_A_FMTP_H264_SPROP_PARAMETER_SETS)
					{
						snprintf(media->avformats[i].extradata, sizeof(media->avformats[i].extradata), "%s", h264.sprop_parameter_sets);
						media->avformats[i].extradatasize = strlen(h264.sprop_parameter_sets);
						//printf("[david debug] i:%d extradata:%s size:%d\n", i, media->avformats[i].extradata, media->avformats[i].extradatasize);
					}
				}
				else if (0 == strcmp("H265", media->avformats[i].encoding))
				{
					...
					media->avformats[i].extradatasize = 0;
					if (h265.flags & SDP_A_FMTP_H265_SPROP_SPS)
					{
						snprintf(media->avformats[i].extradata+media->avformats[i].extradatasize, 
							sizeof(media->avformats[i].extradata)-media->avformats[i].extradatasize, "%s", h265.sprop_sps);
						media->avformats[i].extradatasize += strlen(h265.sprop_sps);
						//printf("[david debug] extradata:%s size:%d 11\n", media->avformats[i].extradata, media->avformats[i].extradatasize);
					}
						
					if (h265.flags & SDP_A_FMTP_H265_SPROP_PPS)
					{
						snprintf(media->avformats[i].extradata+media->avformats[i].extradatasize, 
							sizeof(media->avformats[i].extradata)-media->avformats[i].extradatasize, ",%s", h265.sprop_pps);
						media->avformats[i].extradatasize += strlen(h265.sprop_pps)+1;
						//printf("[david debug] extradata:%s size:%d 22\n", media->avformats[i].extradata, media->avformats[i].extradatasize);
					}
					if (h265.flags & SDP_A_FMTP_H265_SPROP_VPS)
					{
						snprintf(media->avformats[i].extradata+media->avformats[i].extradatasize, 
							sizeof(media->avformats[i].extradata)-media->avformats[i].extradatasize, ",%s", h265.sprop_vps);
						media->avformats[i].extradatasize += strlen(h265.sprop_vps)+1;
						//printf("[david debug] extradata:%s size:%d 33\n", media->avformats[i].extradata, media->avformats[i].extradatasize);
					}
				}
