...
static int rtsp_client_media_teardown(struct rtsp_client_t* rtsp, int i)
{
	...
	//wyl modify for do not have aggregate control uri send rtsp orgin url to teardown(old version is : rtsp->media[i].uri)
	r = rtsp_client_authenrization(rtsp, "TEARDOWN", rtsp->uri, NULL, 0, rtsp->authenrization, sizeof(rtsp->authenrization));
	r = snprintf(rtsp->req, sizeof(rtsp->req), sc_format, rtsp->uri, rtsp->cseq++, rtsp->session[i].session, rtsp->authenrization, USER_AGENT);
