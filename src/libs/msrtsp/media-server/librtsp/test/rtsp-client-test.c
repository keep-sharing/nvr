...
void rtp_receiver_tcp_input(void **s_ctx, uint8_t channel, const void* data, uint16_t bytes);
void rtp_receiver_test(socket_t rtp[2], const char* peer, int peerport[2], int payload, const char* encoding, int uid, void *param);
void* rtp_receiver_tcp_test(void **s_ctx, uint8_t interleave1, uint8_t interleave2, int payload, const char* encoding, int uid, void *param);
...
//	static int keepalive = 0;
	struct rtsp_client_test_t *ctx = (struct rtsp_client_test_t *)param;
	rtp_receiver_tcp_input(ctx->s_ctx, channel, data, bytes);
//	if (++keepalive % 1000 == 0)
//	{
...
static int onsetup(void* param)
{
	...
	for (i = 0; i < rtsp_client_media_count(ctx->rtsp); i++)
	{
		...
		if (RTSP_TRANSPORT_RTP_UDP == transport->transport)
		{
			...
			if (*transport->source)
			{
				rtp_receiver_test(ctx->rtp[i], transport->source, port, payload, encoding, ctx->uid, ctx->rtsp);
			}
			else
			{
				socket_getpeername(ctx->socket, ip, &rtspport);
				rtp_receiver_test(ctx->rtp[i], ip, port, payload, encoding, ctx->uid, ctx->rtsp);
			}
		}
		else if (RTSP_TRANSPORT_RTP_TCP == transport->transport)
		{
			...
			rtp_receiver_tcp_test(ctx->s_ctx, transport->interleaved1, transport->interleaved2, payload, encoding, ctx->uid, ctx->rtsp);
		}
		else
		{
			assert(0); // TODO
...
void rtsp_client_test(const char* host, const char* file, int uid, int transport)
{
	int r, dosleep = 0;
...
//	char packet[2 * 1024 * 1024];
    ...
    ctx.uid = uid;
	ctx.transport = transport; // RTSP_TRANSPORT_RTP_TCP
	snprintf(ppacket, size, "rtsp://%s/%s", host, file); // url
//    printf("=111=[%d]====host[%s]==file[%s]=[%s]==\n", uid, host, file, 
//            ctx.transport == RTSP_TRANSPORT_RTP_TCP ? "tcp" : "udp");
	socket_init();
	ctx.socket = socket_connect_host(host, 554, 2000);
	socket_setrecvbuf(ctx.socket, 1024*1024);
	assert(socket_invalid != ctx.socket);
	//ctx.rtsp = rtsp_client_create(NULL, NULL, &handler, &ctx);
	ctx.rtsp = rtsp_client_create(ppacket, "admin", "1", &handler, &ctx);
	assert(ctx.rtsp);
	assert(0 == rtsp_client_describe(ctx.rtsp));
	printf("==[%d]====host[%s]==file[%s]=[%s]==\n", uid, host, file, 
	    ctx.transport == RTSP_TRANSPORT_RTP_TCP ? "tcp" : "udp");

	socket_setnonblock(ctx.socket, 0);
	r = socket_recv(ctx.socket, ppacket, size, 0);
	while(r > 0)
	{
		assert(0 == rtsp_client_input(ctx.rtsp, ppacket, r, &dosleep));
		r = socket_recv(ctx.socket, ppacket, size, 0);
	}
