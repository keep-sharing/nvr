...
extern "C" void rtp_receiver_test(socket_t rtp[2], const char* peer, int peerport[2], int payload, const char* encoding, int uid, void *param);
...
static int rtsp_onrecord(void* /*ptr*/, rtsp_server_t* rtsp, const char* uri, const char* session, const int64_t* /*npt*/, const double* /*scale*/)
{
	...
	for (it = streams.begin(); it != streams.end(); ++it)
	{
		...
		if (RTSP_TRANSPORT_RTP_UDP == stream->transport.transport)
		{
			...
			rtp_receiver_test(stream->socket, stream->transport.destination, port, stream->media->avformats[0].fmt, stream->media->avformats[0].encoding, 0, NULL);
		}
