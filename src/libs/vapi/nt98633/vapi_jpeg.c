...
static INT encode_jpg(HD_PATH_ID pathId, UINTPTR yuvPhyAddr, FRAME_T *poutFrame)
{
    HD_RESULT  ret = HD_OK;
    INT bsMaxSize;
    ...
    bsMaxSize = poutFrame->width * poutFrame->height;
	...
	// Need to memset 0 (touch va to obtain actual page) before the following cache operation
	memset(bsOutBuffer.va, 0, bsMaxSize);
    ...
    if (ret != HD_OK) {
        ...
        if (HD_OK != vendor_common_mem_cache_sync(bsOutBuffer.va, bsMaxSize, VENDOR_COMMON_MEM_DMA_FROM_DEVICE)) {
            VAPILOG("hd_common_mem_cache_sync fail\n");
            goto exit;
        }
        pack_num  = 0; // 0 for jpeg
        pa_offset = videoBitstream.video_pack[pack_num].phy_addr - bsOutBuffer.pa;
        size      = videoBitstream.video_pack[pack_num].size;
        data      = ms_malloc(size);
        if (data == NULL) {
            VAPILOG("malloc fail\n");
            goto exit;
        }
        memcpy(data, bsOutBuffer.va + pa_offset, size);
    }
...
static HD_RESULT set_encoder(HD_PATH_ID vencPath, UINT32 quality, UINT32 width, UINT32 height)
{
    ...
    videoInParam.pxl_fmt = HD_VIDEO_PXLFMT_YUV420;
                                ...
                                FRAME_T *poutFrame, HI_BOOL isDecRelease)
{
    ...
    UINTPTR scaleOutPa = 0;
    UINTPTR scaleOutVa = 0;
    UINTPTR scaleInPa = 0;
    UINTPTR scaleInVa = 0;
    ...
    */
    ...
    /* prepare scale_in_buffer */
    memcpy(&scaleInBuffer, &outBuffer, sizeof(HD_VIDEO_FRAME));
    ...
    scaleOutVa = (UINTPTR)hd_common_mem_mmap(HD_COMMON_MEM_MEM_TYPE_NONCACHE, scaleOutPa, scaleOutBlkSize);
    ...
    scaleOutBuffer.pxlfmt = HD_VIDEO_PXLFMT_YUV420;

    /* Push videoproc buffer */
    ...
    /* Release dec_out buffer */
    if (isDecRelease) {
        ret = hd_videodec_release_out_buf(vdecPath, &outBuffer);
        if (ret != HD_OK) {
            VAPILOG("hd_videodec_release_out_buf fail 1\n");
            goto exit_enc;
        }
    }
...
static HD_RESULT vapi_h26x_to_jpeg(HD_PATH_ID vdecPath, FRAME_T *pinFrame, FRAME_T *poutFrame)
{
    ...
    decConfig.max_mem.dim.w = ALIGN_CEIL(pinFrame->width, 64);
    decConfig.max_mem.dim.h = ALIGN_CEIL(pinFrame->height, 64);
