...
//        VAPILOG("vdec channel[%d] has existed !\n", vdChn);
    ...
    decConfig.max_mem.dim.w = ALIGN_CEIL(u32Width, 64);
    decConfig.max_mem.dim.h = ALIGN_CEIL(u32Height, 64);
...
{
    *W = ALIGN_CEIL(g_stVdec.stChn[vdChn].stFormat.width, 8);
    *H = ALIGN_CEIL(g_stVdec.stChn[vdChn].stFormat.height, 2);
}
