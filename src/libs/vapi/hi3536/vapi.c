...
static inline HI_S32 ive_copy(int srcPhy, void *srcVir, int dstPhy, void *dstVir, 
                                HI_S32 offset, HI_U16 u16Width, HI_U16 u16Height)
{
	IVE_DMA_CTRL_S stDmaCtrl = { IVE_DMA_MODE_DIRECT_COPY, 0};
 	IVE_HANDLE iveHandle;
	IVE_SRC_DATA_S iveSrc;
	IVE_DST_DATA_S iveDst;
	HI_S32 s32Ret;

    iveSrc.u16Width   = u16Width;
    iveSrc.u16Height  = u16Height;
    iveSrc.u16Stride  = iveSrc.u16Width;
    iveSrc.pu8VirAddr = srcVir + offset;
    iveSrc.u32PhyAddr = srcPhy + offset;
    
    iveDst.pu8VirAddr = dstVir + offset;
    iveDst.u32PhyAddr = dstPhy + offset;
    iveDst.u16Width   = iveSrc.u16Width;
    iveDst.u16Height  = iveSrc.u16Height;
    iveDst.u16Stride  = iveSrc.u16Stride;
    s32Ret = HI_MPI_IVE_DMA(&iveHandle, &iveSrc, &iveDst, &stDmaCtrl, HI_TRUE);
...
static inline HI_S32 vapi_ive_copy(int srcPhy, void *srcVir, int dstPhy, void *dstVir, int width, int height)
{
	HI_S32 i, n, m;
	HI_S32 s32Ret;
	HI_S32 offset = 0;
	...
	for (i = 0 ; i < n; i++)
	{
        s32Ret = ive_copy(srcPhy, srcVir,
                          dstPhy, dstVir, offset, 
                          MAX_IVE_DMA_WIDTH, MAX_IVE_DMA_HEIGHT);
		if (s32Ret != HI_SUCCESS)
			return HI_FAILURE;
...
static inline HI_S32 vapi_vgs_scale(VIDEO_FRAME_INFO_S *pstVoutFrame, HI_U32 VinPhyAddr, HI_U32 width, HI_U32 height)
{
    VGS_HANDLE hHandle;
    ...
    if(s32Ret != HI_SUCCESS)
    {   
        VAPILOG("HI_MPI_VGS_BeginJob fail for %#x!\n", s32Ret);
        return s32Ret;
    }
    
    memcpy(&stTask.stImgIn, pstVoutFrame, sizeof(VIDEO_FRAME_INFO_S));
    stTask.stImgIn.stVFrame.u32Width = width;
    stTask.stImgIn.stVFrame.u32Height = height;
    stTask.stImgIn.stVFrame.u32PhyAddr[0] = VinPhyAddr;
    stTask.stImgIn.stVFrame.u32PhyAddr[1] = VinPhyAddr + (width * height);
    stTask.stImgIn.stVFrame.u32Stride[0] = width;
    stTask.stImgIn.stVFrame.u32Stride[1] = width;
    memcpy(&stTask.stImgOut, pstVoutFrame, sizeof(VIDEO_FRAME_INFO_S));
    
    s32Ret = HI_MPI_VGS_AddScaleTask(hHandle, &stTask);
    if(s32Ret != HI_SUCCESS)
    {   
        HI_MPI_VGS_CancelJob(hHandle);
        VAPILOG("HI_MPI_VGS_AddScaleTask fail for %#x!\n", s32Ret);
        return s32Ret;
    }
    ...
    {   
        HI_MPI_VGS_CancelJob(hHandle);
        VAPILOG("HI_MPI_VGS_EndJob fail for %#x!\n", s32Ret);
        return s32Ret;
    }
...
int vapi_vpool_new(int blkSize, int blkCnt, char *poolName)
{
    ...
    poolId = HI_MPI_VB_CreatePool(blkSize, blkCnt, poolName);
    if (VB_INVALID_POOLID == poolId)
    {
        VAPILOG("HI_MPI_VB_CreatePool failed with %#x!\n", poolId);
        return HI_FAILURE;
    }
    ...
    {
        VAPILOG("HI_MPI_VB_MmapPool failed with %#x!\n", s32Ret);
        return s32Ret;
    }
    return poolId;
}
...
int vapi_vpool_delete(int poolId)
{
    ...
    s32Ret = HI_MPI_VB_MunmapPool(poolId);
    if (HI_SUCCESS != s32Ret)
    {
        VAPILOG("HI_MPI_VB_MunmapPool failed with %#x!\n", poolId);
        return s32Ret;
    }
    s32Ret = HI_MPI_VB_DestroyPool(poolId);
    if (HI_SUCCESS != s32Ret)
    {
        VAPILOG("HI_MPI_VB_DestroyPool failed with %#x!\n", poolId);
        return s32Ret;
    }
    return s32Ret;
}
...
int vapi_fish_vo(SCREEN_E enScreen, int VoChn, int poolId, int inWidth, int inHeight, void *pVirAddr, int phyAddr)
{
    ...
    HI_U32 u32BlkHandle, u32PhyAddr;
    void  *pu8VirAddr;
    VO_LAYER VoLayer;
    HI_U32 outWidth, outHeight;
    HI_U32 s32Ret = HI_SUCCESS;
    ...
    if (s32Ret != HI_SUCCESS)
    {
        VAPILOG("HI_MPI_VO_GetChnAttr failed with %#x!\n", s32Ret);
        return s32Ret;
    }

    outWidth = stChnAttr.stRect.u32Width;
    outHeight = stChnAttr.stRect.u32Height;
        
    u32BlkHandle = HI_MPI_VB_GetBlock(poolId, 0, NULL);
    if (VB_INVALID_HANDLE == u32BlkHandle)
    {
        VAPILOG("getBlock fail %#x!\n", u32BlkHandle);
        return HI_FAILURE;
    }
    
    u32PhyAddr = HI_MPI_VB_Handle2PhysAddr(u32BlkHandle);
    s32Ret = HI_MPI_VB_GetBlkVirAddr(poolId, u32PhyAddr, &pu8VirAddr);
    if(s32Ret != HI_SUCCESS)
    {
        HI_MPI_VB_ReleaseBlock(u32BlkHandle);
        VAPILOG("HI_MPI_VB_GetBlkVirAddr fail for %#x!\n", s32Ret);
        return s32Ret;
    }

    stVoFrame.u32PoolId = poolId;
    stVoFrame.stVFrame.enCompressMode = COMPRESS_MODE_NONE;
    stVoFrame.stVFrame.enVideoFormat = VIDEO_FORMAT_LINEAR;
    stVoFrame.stVFrame.enPixelFormat = PIXEL_FORMAT_YUV_SEMIPLANAR_420;
    stVoFrame.stVFrame.u32Field = VIDEO_FIELD_FRAME;
    stVoFrame.stVFrame.u64pts = 0;
    stVoFrame.stVFrame.u32PhyAddr[0] = u32PhyAddr;

    if (outWidth > inWidth || outHeight > inHeight)
    {
        stVoFrame.stVFrame.u32Width = outWidth;
        stVoFrame.stVFrame.u32Height = outHeight;
        stVoFrame.stVFrame.u32PhyAddr[1] = u32PhyAddr + (outWidth * outHeight);
        stVoFrame.stVFrame.u32Stride[0] = outWidth;
        stVoFrame.stVFrame.u32Stride[1] = outWidth;
        vapi_vgs_scale(&stVoFrame, phyAddr, inWidth, inHeight);

    }
    else
    {
        stVoFrame.stVFrame.u32Width = inWidth;
        stVoFrame.stVFrame.u32Height = inHeight;
        stVoFrame.stVFrame.u32PhyAddr[1] = u32PhyAddr + (inWidth * inHeight);
        stVoFrame.stVFrame.u32Stride[0] = inWidth;
        stVoFrame.stVFrame.u32Stride[1] = inWidth;
        vapi_ive_copy(phyAddr, pVirAddr, u32PhyAddr, pu8VirAddr, inWidth, inHeight);
    }

    s32Ret = HI_MPI_VO_SendFrame(VoLayer , VoChn, &stVoFrame, 0);
    if (s32Ret != HI_SUCCESS)
    {
        VAPILOG("HI_MPI_VO_SendFrame failed with %#x!\n", s32Ret);
    }
...
int vapi_fish_src_wh(SCREEN_E enScreen, int VoChn, int *pInWidth, int *pInHeight, int *pOutWidth, int *pOutHeight)
{
    ...
    if (vapi_vpss_chn_get_wh(g_stVpssGrp.stBind[Grp].vpssGrp, Chn, (HI_U32 *)pInWidth, (HI_U32 *)pInHeight) != HI_SUCCESS)
        return HI_FAILURE;

    if (vapi_vo_chn_get_wh(g_stVpssGrp.stBind[Grp].voutLayer, VoChn, (HI_U32 *)pOutWidth, (HI_U32 *)pOutHeight) != HI_SUCCESS)
        return HI_FAILURE;
