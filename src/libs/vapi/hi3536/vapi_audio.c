...
static HI_S32 vapi_aio_set_codec(AUDIO_SAMPLE_RATE_E enSample)
{
    ...
    if (ioctl(fdAcodec, ACODEC_ENABLE_BOOSTL, &boostl)) {
        VAPILOG("ACODEC_ENABLE_BOOSTL(%d) failed \n", boostl);
        ret = HI_FAILURE;
    }
    if (ioctl(fdAcodec, ACODEC_SET_GAIN_MICL, &gain_mic)) {
        VAPILOG("ACODEC_SET_GAIN_MICL(%d) failed \n", gain_mic);
        ret = HI_FAILURE;
    }
    if (ioctl(fdAcodec, ACODEC_SET_GAIN_MICR, &gain_mic)) {
        VAPILOG("ACODEC_SET_GAIN_MICR(%d) failed \n", gain_mic);
        ret = HI_FAILURE;
    }
                                ...
                                HI_U32 u32ChnCnt, AUDIO_SAMPLE_RATE_E enOutSamplerate)
{
    ...
    for (i = 0; i < u32ChnCnt; i++) {
        ...
        if (g_stAioInfo.stAiDev.bVqeEn == HI_TRUE) {
            ...
            stVqeConfig.bAecOpen            = HI_TRUE;
            stVqeConfig.stAecCfg.bUsrMode   = HI_TRUE;
            stVqeConfig.stAecCfg.s8CngMode  = 0;
            stVqeConfig.stAecCfg.s8NearAllPassEnergy  = 2;
            stVqeConfig.stAecCfg.s8NearCleanSupEnergy = 2;
            stVqeConfig.stAecCfg.s16DTHnlSortQTh  = 16384;

            stVqeConfig.stAecCfg.s16EchoBandLow   = 10;         //\D3\EF\D2\F4\B4\A6\C0\EDƵ\B6\CE 1,\B5\CDƵ\B2\CE\CA\FD
            stVqeConfig.stAecCfg.s16EchoBandHigh  = 23;         //\D3\EF\D2\F4\B4\A6\C0\EDƵ\B6\CE 1,\B8\DFƵ\B2\CE\CA\FD
            stVqeConfig.stAecCfg.s16EchoBandLow2  = 27;         //\D3\EF\D2\F4\B4\A6\C0\EDƵ\B6\CE 2,\B5\CDƵ\B2\CE\CA\FD
            stVqeConfig.stAecCfg.s16EchoBandHigh2 = 45;         //\D3\EF\D2\F4\B4\A6\C0\EDƵ\B6\CE 2,\B8\DFƵ\B2\CE\CA\FD
            ...
            stVqeConfig.bAgcOpen            = HI_FALSE;
            stVqeConfig.stAgcCfg.bUsrMode   = HI_FALSE;
            stVqeConfig.bAnrOpen            = HI_TRUE;
            stVqeConfig.stAnrCfg.bUsrMode   = HI_FALSE;
...
HI_S32 vapi_aio_ao_write_frame(AUDIO_DEV AoDevId, AIO_FRAME_S *pstFrame)
{
    ...
    for (i = 0; i < g_stAioInfo.stAoDev[AoDevId].ChnCnt; i++) {
        ...
        if (HI_SUCCESS != s32Ret) {
            ...
            VAPILOG("HI_MPI_AO_SendFrame(%d, %d), stData.u32Len(%d), failed with %#x! \n", AoDevId, i, stData.u32Len, s32Ret);
            comm_mutex_unlock(&g_stAioInfo.stAoDev[AoDevId].mutex);
            return HI_FAILURE;
