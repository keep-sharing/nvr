...
static HI_S32 vapi_vo_start_dev(VO_DEV VoDev, VO_INTF_SYNC_E enIntfSync, int isogeny)
{
    HI_S32 s32Ret = HI_SUCCESS;
...
HI_S32 vapi_vo_chn_get_wh(VO_LAYER VoLayer, VO_CHN VoChn, HI_U32 *pu32Width, HI_U32 *pu32Height)
{
    HI_S32 s32Ret = HI_SUCCESS;
    VO_CHN_ATTR_S stChnAttr;
    ...
    if (s32Ret != HI_SUCCESS)
    {
        VAPILOG("HI_MPI_VO_GetChnAttr[%d][%d] failed with %#x!\n", VoLayer, VoChn, s32Ret);
        return HI_FAILURE;
    }
    *pu32Width = stChnAttr.stRect.u32Width;
    *pu32Height = stChnAttr.stRect.u32Height;
...
HI_S32 vapi_vo_dev_open(VO_DEV VoDev, SCREEN_RES_E enRes)
{
    ...
    if (s32Ret != HI_SUCCESS)
    {
        VAPILOG("vapi_vo_dev_open [%d] isogeny[%d] failed with %#x!\n", VoDev, g_stVoChn.isogeny, s32Ret);
        return HI_FAILURE;
    }    
...
HI_S32 vapi_vo_init(SCREEN_RES_E enMain, SCREEN_RES_E enSub, int isHotplug, int isDualScreen, int isogeny, VDEC_CB_S *pstCb)
{
    HI_S32 s32Ret = HI_SUCCESS;
