...
static HD_RESULT aio_ai_start(HI_S32 aiDevId, ARATE_E enInSampleRate)
{
    ...
    aiCfg.data_pool[0].counts = HD_AUDIOCAP_SET_COUNT(10, 0);
...
static HD_RESULT aio_ao_start(AO_E aoDevId, ARATE_E enOutSampleRate)
{
    HD_RESULT ret = HD_OK;
    ...
    adCfg.data_pool[0].frame_sample_size = 1024 * 2;
    ...
    g_stAioInfo.stAoDev[aoDevId].sampleRate =
        aoDevId == AO_MAIN_HDMI ? DEFAULT_SAMPLE_RATE : enOutSampleRate;
...
#endif
    aio_ao_write(aoDevId, pstFrame->enArate, dst, dstSize);
    comm_mutex_unlock(&g_stAioInfo.stAoDev[aoDevId].mutex);
...
HD_RESULT vapi_aio_init()
{
    ...
    if ((ret = aio_ao_start(AO_SPK_MIC, DEFAULT_SAMPLE_RATE)) != HD_OK) {
        VAPILOG("aio_ao_start AO_SPK_MIC fail\n");
        return ret;
    }

    if ((ret = aio_ao_start(AO_MAIN_HDMI, ARATE_48000)) != HD_OK) {
