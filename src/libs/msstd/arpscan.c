...
static struct {
	char     device[32];  /* name of interface to scan from */
	char     network[32]; /* network (CIDR) to scan */
...
static void recv_arp_replies(uint32_t timeout, char *dstmac, int ndstsize);
...
static void
for_each_only_ip(const char *ip, ip_callback cb)
{
	struct in_addr ipv4;
	if (inet_pton(AF_INET, ip, &ipv4) != 1) {
		fprintf(stderr, "failed to parse as an IP address\n");
		return;
	}
	//printf("[david debug] ip:%s\n", ip);
	(*cb)(&ipv4);
...
static void
recv_arp_replies(uint32_t timeout, char *dstmac, int ndstsize)
{
...
#endif		
		snprintf(dstmac, ndstsize, ETHER_MAC_FMT, 
			/* Ethernet MAC */
			(uint8_t)(ARP.arp_sha[0]), (uint8_t)(ARP.arp_sha[1]),
			(uint8_t)(ARP.arp_sha[2]), (uint8_t)(ARP.arp_sha[3]),
			(uint8_t)(ARP.arp_sha[4]), (uint8_t)(ARP.arp_sha[5]));
...
show_version(void)
{
	printf("arpscan Copyright 2014 James Hunt\n");
}
#endif
static void
...
int arpscan_ipaddr(const char *dev, const char *ipaddr, char *dstmac, int ndstsize)
{
...
#else
	snprintf(OPTIONS.device, sizeof(OPTIONS.device), "%s", dev);
	snprintf(OPTIONS.network, sizeof(OPTIONS.network), "%s", ipaddr);
	...
	if (WANT_INFO) fprintf(stderr, "arpscan v starting up.\n");
	if (WANT_DEBUG) {
	...
	for_each_only_ip(OPTIONS.network, send_arp_request);

	if (WANT_INFO) fprintf(stderr, "Preparing to receive ARP replies\n");
	recv_arp_replies(OPTIONS.timeout * 1000, dstmac, ndstsize);

	if (WANT_INFO) fprintf(stderr, "arpscan shutting down\n");
	close(SOCK);
	return 0;
}
