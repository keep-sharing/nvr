...
#define MS_POWER_DOWN    0xf30cff01
...
void Hi_IR_STRONG_TEST_001()
{
    ...
    scanf("%d", &i);
    (tmp.frequence) = (U8)i;
    printf("input codetype = ");
    scanf("%d", &i);
    (tmp.codetype) = (U8)i;
    printf("input code_len = ");
    scanf("%d", &i);
    (tmp.code_len) = (U8)i;
    printf("input leads_min = ");
    scanf("%d", &i);
    (tmp.leads_min) = (U8)i;
    printf("input leads_max = ");
    scanf("%d", &i);
    (tmp.leads_max) = (U8)i;
    printf("input leade_min = ");
    scanf("%d", &i);
    (tmp.leade_min) = (U8)i;
    printf("input leade_max = ");
    scanf("%d", &i);
    (tmp.leade_max) = (U8)i;
    printf("input cnt0_b_min = ");
    scanf("%d", &i);
    (tmp.cnt0_b_min) = (U8)i;
    printf("input cnt0_b_max = ");
    scanf("%d", &i);
    (tmp.cnt0_b_max) = (U8)i;
    printf("input cnt1_b_min = ");
    scanf("%d", &i);
    (tmp.cnt1_b_min) = (U8)i;
    printf("input cnt1_b_max = ");
    scanf("%d", &i);
    (tmp.cnt1_b_max) = (U8)i;
    printf("input sleade_min = ");
    scanf("%d", &i);
    (tmp.sleade_min) = (U8)i;
    printf("input sleade_max = ");
    scanf("%d", &i);
    (tmp.sleade_max) = (U8)i;
...
static int  test_3536_ir_start(void)
{
    /* NEC with full repeat code */
    int filp = 0;
    int res = 0;
    ...
    printf("\n");
    printf("please press a power key to pass the testing !\n");
    while (!g_exit) {
        FD_ZERO(&read_fds);
        FD_SET(filp, &read_fds);
        TimeoutVal.tv_sec  = 1;
        TimeoutVal.tv_usec = 0;
        res = select(filp + 1, &read_fds, NULL, NULL, &TimeoutVal);
        if (res > 0) {
            if (FD_ISSET(filp, &read_fds)) {
                res = read(filp, &rcv_irkey_info, sizeof(rcv_irkey_info));
                if ((res > 0) && (res <= sizeof(rcv_irkey_info))) {
//                    normal_report_irkey(rcv_irkey_info);
                    if (rcv_irkey_info.irkey_datal == MS_POWER_DOWN) {
                        SHOW_YES;
                        g_exit = 1;
                    } else {
                        printf("please press a power key to pass the testing !\n");
                    ...
                    hderr("Error. read irkey device error. res=%d.\n", res);
                }
            }
...
#ifdef _HI3798_
static int test_3798c_ir_start()
{
    int res = 0;
    ...
    res = HI_UNF_IR_Init();
    if (res) {
        hderr("Fail to open ir dev!\n");
    }

    HI_UNF_IR_SetCodeType(HI_UNF_IR_CODE_NEC_FULL);
    res = HI_UNF_IR_SetFetchMode(0);
    if (res) {
        hderr("Fail to set key fetch mode!\n");
        return -1;
    }
    ...
    while (!g_exit) {
        ...
        res = HI_UNF_IR_GetValueWithProtocol(&status, &key, name, sizeof(name), 200);
        if (!res) {
            if (key == MS_POWER_DOWN && status == HI_UNF_KEY_STATUS_DOWN) {
                HI_UNF_IR_Reset();
                SHOW_YES;
                g_exit = 1;
            }
...
HD_MODULE_INFO(TEST_NAME_IR, test_find, test_start, test_stop);
