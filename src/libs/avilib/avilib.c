...
static ssize_t avi_write (int fd, char *buf, size_t len)
{
   ...
   while (r < len) {
	  ...
	  //fdatasync(fd);//bruce.milesight debug.not use.
      if ((ssize_t)n < 0)
         return n;
...
#define AVIF_COPYRIGHTED        0x00020000
   ...
   for(j=0; j<AVI->anum; ++j) {
       ...
       // ThOe /4
	   if(AVI->track[j].a_fmt == WAVE_FORMAT_AAC)
	   {/*AAC rate/scale = samplerate/(1024*samplesizeperframe) 
		  samplesizeperframe delegate one aac frame have how many bytes audio samples as sample size, all most is 1*/
		  OUTLONG(16); /*Scale*/
		  OUTLONG(125*(AVI->track[j].a_rate / 8000));// rate
	   }
	   else
	   {
       ...
       OUTLONG(1000*AVI->track[j].mp3rate/8);
       //ThOe (/4)
		if(AVI->track[j].a_fmt == WAVE_FORMAT_AAC)//wyl add from ffmpeg
	   {
		   OUTSHRT(768*AVI->track[j].a_chans); /* BlockAlign */
		}
		else
...
static int avi_write_data(avi_t *AVI, char *data, unsigned long length, int audio, int keyframe)
{
   ...
   if(audio)
     n = avi_add_index_entry(AVI,astr,AVI->track[AVI->aptr].a_fmt == WAVE_FORMAT_AAC?0x10:0x0,AVI->pos,length);
   else
...
int AVI_write_audio(avi_t *AVI, char *data, long bytes)
{
   ...
   //wyl add for aac
   if(AVI->track[AVI->aptr].a_fmt == WAVE_FORMAT_AAC)
...
int AVI_close(avi_t *AVI)
{
   ...
   /* Even if there happened an error, we first clean up */
	//fdatasync(AVI->fdes);//bruce.milesight debug.not use.
	//posix_fadvise(AVI->fdes, 0, 0, POSIX_FADV_DONTNEED);
   close(AVI->fdes);
   if(AVI->idx)
   	{
		free(AVI->idx);
...
avi_t *AVI_open_input_file(const char *filename, int getIndex)
{
  ...
  ret = avi_parse_input_file(AVI, getIndex);
  if(!ret)
  	AVI->aptr=0; //reset  
...
//   fprintf(stderr, "chunks = %ld %d %s\n", AVI->track[0].audio_chunks, AVI->anum, AVI->track[0].audio_tag);
   ...
   for(i=0;i<AVI->n_idx;i++) {
	 ...
	 AVI->track[j].audio_index[nai[j]].videopos = nvi;//wyl.milesight
...
//wyl.milesight add
long AVI_read_audio_chunk_with_videopos(avi_t *AVI, char *audbuf, long videoframe)
{
   long pos, left;
   ...
   if(left==0||videoframe < AVI->track[AVI->aptr].audio_index[AVI->track[AVI->aptr].audio_posc].videopos) return 0;
