...
struct mov_writer_t* mov_writer_create(const struct mov_buffer_t* buffer, void* param, int flags,uint32_t mdat_size)
{
	struct mov_t* mov;
	...
	mov_buffer_w32(&mov->io, mdat_size); /* size */
...
//	}
	...
	if (MOV_FLAG_FASTSTART & mov->flags)
	{
		...
		uint64_t buff_tell = mov_buffer_tell(&mov->io);
		if (buff_tell != offset2 + co64)
		{
			printf("may be the frame time usec has same problems\n");
		}
		//offset2 = mov_buffer_tell(&mov->io);
...
uint64_t mov_get_mdat_size(mov_writer_t* mov)
{
	return mov->mdat_size;
}
