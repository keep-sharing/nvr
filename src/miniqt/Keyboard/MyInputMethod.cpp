...
{
    m_widgetKeyboard = new KeyboardWidget(nullptr);
    m_widgetKeyboard->hide();
}
...
void MyInputMethod::updateHandler(int type)
{
    ...
    case FocusIn: {
        ...
        if (isShow) {
            ...
            if (point.x() + m_widgetKeyboard->width() > screenRect.right()) {
                point.setX(screenRect.right() - m_widgetKeyboard->width());
            }
            if (point.y() + m_widgetKeyboard->height() > screenRect.bottom()) {
                point.setY(point.y() - m_widgetKeyboard->height());
            } else {
...
{
    m_widgetKeyboard->move(pos);
    m_widgetKeyboard->show();
}
