    ...
    if (watched == this) {
        if (event->type() == QEvent::MouseButtonPress) {
            ...
            if (mouseEvent->buttons() & Qt::LeftButton) {
                dragPosition = mouseEvent->globalPos() - pos();
                dragThreshold.restart();
            }
            return true;
        }
        if (event->type() == QEvent::MouseButtonRelease) {
            dragThreshold.invalidate();
            return true;
        }
        if (event->type() == QEvent::MouseMove) {
            QMouseEvent *mouseEvent = static_cast<QMouseEvent *>(event);
            if ((mouseEvent->buttons() & Qt::LeftButton) && dragThreshold.isValid() && dragThreshold.elapsed() > 200)
