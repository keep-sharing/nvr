...
UBOOT_DIR 		= $(HIBOOT_DIR)/fastboot
PRODUCT_DIR		=$(HIBOOT_DIR)/product
LIBSPRODUCT    = $(PRODUCT_DIR)/libproduct.a
...
UBOOT_VER_FILE	:= $(UBOOT_DIR)/include/configs/hi3798cv2x.h
BOOTCONFIG := hi3798cv2x
UBOOT_REG_CFG	:= reg_info_$(DDR_NAME).reg

include $(UBOOT_DIR)/config.mk
ENVFLAGS 		:= -Wall $(PLATFORM_FLAGS) -DUSE_HOSTCC -I$(UBOOT_DIR)/include -I$(ENV_SRC_DIR)
ifeq ($(MTD_VERSION),old)
ENVFLAGS 		+= -DMTD_OLD
endif
HIBOOT_FLAGS 	:= $(MS_MAKE_JOBS) O=$(UBOOT_DIR) ARCH=arm CROSS_COMPILE=$(CROSS_COMPILE) LIBSPRODUCT=$(LIBSPRODUCT) CONFIG_MERGE_IMAGE=

ifeq ($(DDR_SIZE),$(DDR_SIZE_2G))
UBOOT_DDR_SIZE	= (2*1024*1024*1024)
else
UBOOT_DDR_SIZE	= (1*1024*1024*1024)
endif
UBOOT_CMDLINE	= mem=$(ARM_DDR_SIZE) console=ttyAMA0,115200 ubi.mtd=fs1 root=ubi0:rootfs rw rootfstype=ubifs mmz=ddr,0,0,$(MMZ_SIZE)
...
hiboot-prepare:
	...
	$(MS_MAKEFILE_V)cp $(HIBOOT_DIR)/$(UBOOT_REG_CFG) $(UBOOT_DIR)/reg_info.reg

product:
	$(MS_MAKEFILE_V)$(MAKE) -C $(UBOOT_DIR) $(HIBOOT_FLAGS) $(BOOTCONFIG)_config
	$(MS_MAKEFILE_V)$(MAKE) -C $(PRODUCT_DIR) $(HIBOOT_FLAGS)
		
hiboot: hiboot-prepare #product
	$(MS_MAKEFILE_V)$(MAKE) -C $(UBOOT_DIR) $(HIBOOT_FLAGS) $(BOOTCONFIG)
	$(MS_MAKEFILE_V)cp $(UBOOT_DIR)/fastboot-burn.bin $(FIRMWARE_IMG_DIR)/uboot_$(DDR_NAME)

hiboot-clean:
	#$(MS_MAKEFILE_V)$(MAKE) -C $(PRODUCT_DIR) $(HIBOOT_FLAGS) clean
	$(MS_MAKEFILE_V)$(MAKE) -C $(UBOOT_DIR) $(HIBOOT_FLAGS) distclean
	-$(MS_MAKEFILE_V)rm -f $(UBOOT_DIR)/reg_info*.reg
