...
#define DEF_CMDLINE		"mem=1024M console=ttyAMA0,115200 ubi.mtd=fs1 root=ubi0:rootfs rw rootfstype=ubifs mmz=ddr,0,0,150M"
...
{
    char defalut_bootarg[128] = DEF_CMDLINE;
    char *cmdline = NULL;
	...
	if( cmdline == NULL )
    {
        memcpy(mtd_table.bootargs, defalut_bootarg, sizeof(defalut_bootarg));        
        cmdline = strstr(defalut_bootarg, "=fs");
        if( cmdline == NULL )
        {
...
//	struct nand_oobinfo old_oobinfo;
	...
	if (!(meminfo.oobsize == 16 && meminfo.writesize == 512) &&
            ...
            !(meminfo.oobsize == 60 && meminfo.writesize == 2048) && //hi3798 add 
            !(meminfo.oobsize == 32 && meminfo.writesize == 2048))
	{
