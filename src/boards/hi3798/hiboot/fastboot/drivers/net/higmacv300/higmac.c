...
static int miiphy_delay(void)
{
    unsigned char phyaddr;
	char *phy_name;

	phyaddr = higmac_board_info[1].phy_addr;
	phy_name = higmac_board_info[1].mii_name;   
	unsigned short reg;
	unsigned int val;
...
int eth_init(bd_t* bd)
{
	...
	printf("higmac_board_info[1].phy_addr = %0x\n",higmac_board_info[1].phy_addr);
	ret = higmac_init();
	phyaddr = higmac_board_info[1].phy_addr;
	phy_name = higmac_board_info[1].mii_name;
	if (ret)
		printf("higmac init fail!\n"); 
	miiphy_read(phy_name, phyaddr,0x02, &val);
	miiphy_read(phy_name, phyaddr,0x02, &val);
	if(val==0x4d)
    miiphy_delay();
