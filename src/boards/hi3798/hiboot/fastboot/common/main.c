...
{
    HI_DRV_GPIO_WriteBit(BUZZ_CTL, BUZZ_ON); // buzzer on
    while(timeout--)
        udelay(100000);
    HI_DRV_GPIO_WriteBit(BUZZ_CTL, BUZZ_OFF); // buzzer off
}
...
static int factory_reset(void)
{
    ...
    HI_DRV_GPIO_ReadBit(RESET_KEY, &value);
    if (value != RESET_KEY_PRESSED)
    {
        return 0;
    }
    else
...
//			rc = -1;
			return -1; //return if cmd failed  modifed by zbing 2016.12.22
		}
