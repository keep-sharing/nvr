...
int download_process_usbhost(void)
{
	...
	/* step1: read bootargs.bin and set env */
	ret = download_bootargs();
	if (ret)
		goto error;

	/* setp2: android update need to replace bootargs from recoverybootargs */
	bootargs_change();

	/* setp3: read recovery.img and go */
