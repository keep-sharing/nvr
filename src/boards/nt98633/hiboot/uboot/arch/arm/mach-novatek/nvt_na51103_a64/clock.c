...
#define CPU_NORMAL_MAX_CLOCK_RATE       1000
#define CPU_OV_CLK_MAX_CLOCK_RATE       1150
...
//			value = 1200;
	...
	case 2:
		...
		case 1100:
			...
			printf("Set CPU clk 1150MHz\n");
			break;

		case 1150:
			//CPU 1150 MHz -> CPU-MPLL-143.75Mhz = 0x17F555
			writel(0x55, IOADDR_CG_REG_BASE + CPU_CLOCK_RATE_RATIO0_OFS); // B0
			writel(0xF5, IOADDR_CG_REG_BASE + CPU_CLOCK_RATE_RATIO1_OFS); // B1
			writel(0x17, IOADDR_CG_REG_BASE + CPU_CLOCK_RATE_RATIO2_OFS); // B2
			printf("Set CPU clk 1150MHz\n");
			break;
...
int prepare_power_trim(int mode)
{
	...
	if (code == -33) {
		...
		power_trim = 0;
		is_found = extract_2_set_type_trim_valid(TRIM_CORE_PWR_ADJ_FIELD, code, (u32 *)&power_trim);
		power_trim &= 0xF;
...
int do_nvt_pwr_ctrl(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	u32 value = simple_strtoul(argv[1], NULL, 10);
	//u32 uiReg;
