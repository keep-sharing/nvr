...
//PLL register move to STBC
#define CLOCK_GEN_PLL_PWR_REG			(IOADDR_STBC_CG_REG_BASE + 0x00)
#define CLOCK_GEN_PLL_STS_REG			(IOADDR_STBC_CG_REG_BASE + 0x04)
#define CLOCK_GEN_ENABLE_REG0           (CLOCK_GEN_REG_BASE_ADDR + 0x70)

// For AXI register
#define CRYPTO_HOST_AXI_CH_DISABLE      (0x001 << 16)
#define CRYPTO_HOST_AXI_CH_ENABLE       (0x000 << 16)

#define CRYPTOHostAXIChannelEnable()    CLRREG32(CRYPTO_REG_BASE_ADDR + 0x68, CRYPTO_HOST_AXI_CH_DISABLE);
#define CRYPTOHostAXIChannelDisable()   SETREG32(CRYPTO_REG_BASE_ADDR + 0x68, CRYPTO_HOST_AXI_CH_DISABLE);
...
#define dma_getPhyAddr(addr)            ((((UINT32)(addr))>=0x80000000UL)?((UINT32)(addr)-0x80000000UL):(UINT32)(addr))
...
static void crypto_dma_enable(u32 uiMode, u32 uiOPMode, u32 uiType, u32 SrcAddr, u32 DstAddr, u32 Len)
{
	...
	CRYPTO_SETREG(CRYPTO_SRC_ADDR_REG_OFS, ((ulong)&vuiSCE_DesTab[0]));
...
s32 crypto_data_operation(EFUSE_OTP_KEY_SET_FIELD key_set, CRYPT_OP crypt_op_param)
{
	...
	reg = INREG32(CLOCK_GEN_PLL_STS_REG);
