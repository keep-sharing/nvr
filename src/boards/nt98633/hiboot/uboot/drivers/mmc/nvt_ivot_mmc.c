...
static void mmc_nvt_parse_driving(struct mmc_nvt_host *host)
{
	...
	/* Config SDIO pinmux*/
	do {
		host->pinmux_value = 0x1; //Default value
		nodeoffset = fdt_path_offset((const void*)fdt_addr, "/top/sdio");
		if (nodeoffset < 0) {
...
static int nvt_emmc_arch_host_preinit(struct mmc_nvt_host *host)
{
	if (host->id == SDIO_HOST_ID_1) {
		...
		*(uint32_t*) 0xF0020084 |= 0x4;
		...
		if (host->pinmux_value & PIN_SDIO_CFG_SDIO_2) {
			*(uint32_t*) 0xF0010004 &= ~0x30;          // SDIO_EN
			udelay(10);
			*(uint32_t*) 0xF0010004 |= 0x20;
			udelay(10);
			*(uint32_t*) 0xF00100A8 &= ~(0x300000);    // PGPIO20~21
			udelay(10);
			*(uint32_t*) 0xF00100B4 &= ~(0x3C0);       // DGPIO6~9
			udelay(10);

			*(uint32_t*) 0xF003000C &= ~(0xF00);       // PGPIO20~21 PU
			*(uint32_t*) 0xF003000C |= 0xA00;
			udelay(10);
			*(uint32_t*) 0xF0030018 &= ~(0xFF000);     // DGPIO6, DGPIO8~9 PU and DGPIO7 PD
			*(uint32_t*) 0xF0030018 |= 0xA6000;
			udelay(10);
			...
			*(uint32_t*) 0xF0010004 |= 0x10;
			udelay(10);
			*(uint32_t*) 0xF00100A8 &= ~(0x8EC000);    // PGPIO14~15, PGPIO17~19, PGPIO23
			udelay(10);

			*(uint32_t*) 0xF0030008 &= ~(0xF0000000);  // PGPIO14~15 PU
			*(uint32_t*) 0xF0030008 |= 0xA0000000;
			*(uint32_t*) 0xF003000C &= ~(0xC0FC);      // PGPIO17, PGPIO19, PGPIO23 PU and PGPIO18 PD
			*(uint32_t*) 0xF003000C |= 0x8098;
			udelay(10);
