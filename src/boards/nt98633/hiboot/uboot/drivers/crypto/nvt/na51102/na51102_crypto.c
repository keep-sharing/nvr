...
#else
#define DMA_ADDR_LO(x)                  ((uint32_t)(x))
#endif
...
#define NVT_CRYPTO_AXI_DMA_DIS_REG		  0x68           ///< crypto AXI control
...
static void nvt_crypto_platform_init(void)
{
	...
	/* Crypto master STBC clock enable */
	tmp = IO_REG_RD(IOADDR_STBC_CG_REG_BASE, 0x00);
	tmp |= (0x1<<2);
	IO_REG_WR(IOADDR_STBC_CG_REG_BASE, 0x00, tmp);
	...
	tmp = IO_REG_RD(IOADDR_CG_REG_BASE, 0x90);
	tmp |= (0x1<<31);
	IO_REG_WR(IOADDR_CG_REG_BASE, 0x90, tmp);
}
...
int nvt_crypto_dma_aes(struct nvt_crypto_pio_t *p_crypto)
{
	...
	/* clear axi dma disable */
	reg_value  = crypto_read(NVT_CRYPTO_AXI_DMA_DIS_REG);
	reg_value &= ~(0x1<<16);
	crypto_write(NVT_CRYPTO_AXI_DMA_DIS_REG, reg_value);
