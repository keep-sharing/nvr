...
int phy_init(void)
{
// add by tct for phy-MAE0621A 2022-3-4
...
int __weak get_phy_id(struct mii_dev *bus, int addr, int devad, u32 *phy_id)
{
    ...
    // add for MAE0621A by tct 2022-3-4
#ifdef CONFIG_PHY_MAXIO
    /*
     ...
     *MII_PHYSID is a read-only register and writine to it has no effect
     */
    bus->write(bus, addr, devad, MII_PHYSID2 , 0x0);
    debug("\n%s PHY(MAE0621A): write MII_PHYSID2", bus->name);
#endif
...
#ifdef CONFIG_MS_NVR
    phy_write(phydev, devad, 0x1e, 0x0c);
    phy_write(phydev, devad, 0x1f, 0x80ab);
#endif
