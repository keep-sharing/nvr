 ...
 * na51102:
 *    NOVATEK's NA51102 chip. This configuration uses an AXI master/DMA bus, an
...
*/
    ...
    reg = readl(IOADDR_CG_REG_BASE + 0x90);
	reg |= (1<<27);	// realse eth0 rst
...
static void extphy0_clk_en(phy_interface_t phy_intf, int phy_clk, int ref_clk_out, int pin_num)
{
	...
	// reset external phy
	gpio_request(D_GPIO(8), "PHY0_RST");
	gpio_direction_output(D_GPIO(8), 0);
	mdelay(10);	// assert low at least 10ms
	gpio_set_value(D_GPIO(8), 1);
...
static void extphy1_clk_en(phy_interface_t phy_intf, int phy_clk, int ref_clk_out , int pin_num)
{
	...
	// reset external phy
	gpio_request(L_GPIO(18), "PHY1_RST"); //L_GPIO(18)
	gpio_direction_output(L_GPIO(18), 0);
	mdelay(10);	// assert low at least 10ms
	gpio_set_value(L_GPIO(18), 1);
	mdelay(50);	// wait 50ms before access phy register
	gpio_free(L_GPIO(18));
...
static void mac0_pinmux_en(phy_interface_t phy_intf, int pin_num)
{
	...
	case PHY_INTERFACE_MODE_RMII:
		...
		*/
		...
		reg = readl(IOADDR_TOP_REG_BASE + 0xB4); // GPIO EN
		reg &= ~(0x00000060);
		writel(reg, IOADDR_TOP_REG_BASE + 0xB4);

		reg = readl(IOADDR_TOP_REG_BASE + 0xB8); // GPIO EN
		reg &= ~(0x1E380000);
		writel(reg, IOADDR_TOP_REG_BASE + 0xB8);
...
static const struct NVT_PLAT_INFO v_nvt_plat_info[2] = {
	...
	// MAC1 (0xF043_0000)
	{
		...
		L_GPIO(18),			// gpio reset L_GPIO(18)
		L_GPIO(18),			// gpio reset L_GPIO(18)
	},
};
...
int na51102_eth_initialize(bd_t *bis)
{
	int ret = 0;
	u32 i;
	char path[20] = {0};
