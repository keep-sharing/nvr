...
int nvt_pcie_dts_load_refclk_out(char *rc_dts_name)
{
	ulong fdt_addr = nvt_readl((ulong)nvt_shminfo_boot_fdt_addr);
	int nodeoffset;
	int addrcell, sizecell, r, offset;
	...
	cell = (u32*)fdt_getprop((const void*)fdt_addr, nodeoffset, "refclk-out", &len);
	if (len <= 0) {
		printf("%s(%d) len = 0\n",__func__, __LINE__);
		addrcell = 0;
	} else {
	...
	printf("%s: refclk-out result %d\r\n", __func__, addrcell);

	return addrcell;
}

int nvt_pcie_dts_load_tx_boost(char *rc_dts_name)
{
	ulong fdt_addr = nvt_readl((ulong)nvt_shminfo_boot_fdt_addr);
	int nodeoffset;
	int addrcell, sizecell, r, offset;
	...
	cell = (u32*)fdt_getprop((const void*)fdt_addr, nodeoffset, "tx-boost", &len);
	if (len <= 0) {
		printf("%s(%d) len = 0\n",__func__, __LINE__);
		addrcell = 0;
	} else {
	...
	if (addrcell > 3) {
		printf("%s: tx-boost max 3, but dts is %d\r\n", __func__, addrcell);
		addrcell = 3;
	}
	printf("%s: tx-boost result %d\r\n", __func__, addrcell);

	return addrcell;
}

int nvt_pcie_dts_load_auto_preset(char *rc_dts_name)
{
	ulong fdt_addr = nvt_readl((ulong)nvt_shminfo_boot_fdt_addr);
	int nodeoffset;
	int addrcell, sizecell, r, offset;
	...
	cell = (u32*)fdt_getprop((const void*)fdt_addr, nodeoffset, "auto-preset", &len);
	if (len <= 0) {
		printf("%s(%d) len = 0\n",__func__, __LINE__);
		addrcell = 0;
	} else {
	...
	printf("%s: auto-preset result %d\r\n", __func__, addrcell);

	return addrcell;
}

int nvt_pcie_dts_load_l1_det(char *rc_dts_name)
{
        ulong fdt_addr = nvt_readl((ulong)nvt_shminfo_boot_fdt_addr);
        int nodeoffset;
        int addrcell, sizecell, r, offset;
        ...
        cell = (u32*)fdt_getprop((const void*)fdt_addr, nodeoffset, "en-l1-det", &len);
        if (len <= 0) {
                printf("%s(%d) len = 0\n",__func__, __LINE__);
                addrcell = 0;
        } else {
