...
static int nvt_update_partitions(unsigned short FlashType, unsigned int addr, unsigned int size, u64 part_off, u64 PartitionSize)
{
	char command[128];
	u32 align_size = 0;
...
#endif
	    default:
	        printf("%s Not this type of flash type %d %s\r\n", ANSI_COLOR_RED, FlashType, ANSI_COLOR_RESET);
	        return -1;
	}
...
static int nvt_read_partitions(unsigned short FlashType, unsigned int addr, unsigned int size, u64 part_off, unsigned int pat_id, unsigned short embtype)
{
	char command[128];
...
#endif
	default:
		printf("%s Not this type of read flash type %d %s\r\n", ANSI_COLOR_RED, FlashType, ANSI_COLOR_RESET);
		return -1;
	}
...
static int nvt_update_fs_partition(unsigned short FlashType, unsigned int addr, unsigned int size, unsigned int part_off, unsigned int part_size, EMB_PARTITION *pEmb)
{
	char command[128];
	...
	sprintf(command, "nand erase clean 0x%x 0x%x", part_off, part_size);
#else
	...
	printf("Update: %s %s\n", __func__, "JFFS2");
	#if defined(CONFIG_NVT_SPI_NAND) || defined(CONFIG_NVT_SPI_NOR_NAND)
	sprintf(command, "nand write.jffs2 0x%x 0x%x 0x%x", addr, part_off, size);
	run_command(command, 0);
	#elif defined(CONFIG_NVT_SPI_NOR)
	sprintf(command, "sf write 0x%x 0x%x 0x%x", addr, part_off, size);
	run_command(command, 0);
...
static int nvt_update_user_partition(unsigned short FlashType, unsigned int addr, unsigned int size, unsigned int part_off, unsigned int part_size, EMB_PARTITION *pEmb)
{
	char command[128];
	...
	printf("Update: %s %s\n", __func__, "JFFS2");
	#if defined(CONFIG_NVT_LINUX_SPINAND_BOOT) || defined(CONFIG_NVT_LINUX_SPINORNAND_BOOT)
	sprintf(command, "nand write.jffs2 0x%x rootfs%u 0x%x", addr, pEmb->OrderIdx, size);
	run_command(command, 0);
	#else
	sprintf(command, "sf write 0x%x 0x%x 0x%x", addr, part_off, size);
	run_command(command, 0);
...
static int nvt_update_rootfsl_partition(unsigned short FlashType, unsigned int addr, unsigned int size, unsigned int part_off, unsigned int part_size, EMB_PARTITION *pEmb)
{
	char command[128];
...
static int nvt_read_fs_partition(unsigned short FlashType, unsigned int addr, unsigned int size, unsigned int part_off, unsigned int part_size, unsigned int pat_id, EMB_PARTITION *pEmb)
{
	char command[128];
	int ret = 0;
...
#if defined(CONFIG_NVT_UBIFS_SUPPORT) && (defined(CONFIG_NVT_LINUX_RAMDISK_SUPPORT) && (defined(CONFIG_NVT_SPI_NAND)  || defined(CONFIG_NVT_SPI_NOR_NAND)))
	/*test ubifs*/
	printf("Read: %s %s\n", __func__, "UBIFS");
	if(pEmb->EmbType == EMBTYPE_ROOTFS){

		#if (defined(CONFIG_NVT_LINUX_RAMDISK_SUPPORT) && (defined(CONFIG_NVT_SPI_NAND) || defined(CONFIG_NVT_SPI_NOR_NAND)))
			if (pEmb->OrderIdx == 0){
...
static int nvt_read_rootfsl_partition(unsigned short FlashType, unsigned int addr, unsigned int size, unsigned int part_off, unsigned int part_size, unsigned int pat_id, EMB_PARTITION *pEmb)
{
	char command[128];
	int ret = 0;
...
static int nvt_read_user_partition(unsigned short FlashType, unsigned int addr, unsigned int size, unsigned int part_off, unsigned int part_size, unsigned int pat_id, EMB_PARTITION *pEmb)
{
	char command[128];
	int ret = 0;
...
static UINT32 MemCheck_CalcCheckSum16Bit(ulong uiAddr,UINT32 uiLen)
{
	UINT32 i,uiSum = 0;
...
static int nvt_on_partition_enum_sanity(unsigned int id, NVTPACK_MEM* p_mem, void* p_user_data)
{
	...
	unsigned int reserve_blocks = rounddown(PartitionSize - size, nand_get_block_size()) / nand_get_block_size();
...
static int nvt_on_partition_enum_update(unsigned int id, NVTPACK_MEM* p_mem, void* p_user_data)
{
	...
	switch(pEmb[id].EmbType)
	{
		...
		case EMBTYPE_ROOTFSL:
			...
			ret = nvt_update_rootfsl_partition(pEmb[id].FlashType, (ulong)p_mem->p_data, p_mem->len, PartitionOffset, PartitionSize, &pEmb[id]);
			break;
		case EMBTYPE_RTOS:
		...
		case EMBTYPE_USER:
			...
			ret = nvt_update_user_partition(pEmb[id].FlashType, (ulong)p_mem->p_data, p_mem->len, PartitionOffset, PartitionSize, &pEmb[id]);
			break;
...
static int nvt_on_partition_enum_mtd_readback(unsigned int id, NVTPACK_MEM* p_mem, void* p_user_data)
{
	...
	switch(pEmb[id].EmbType)
	{
		...
		case EMBTYPE_ROOTFSL:
			printf("%s Read: rootfs%u %s\r\n", ANSI_COLOR_YELLOW, pEmb[id].OrderIdx, ANSI_COLOR_RESET);
			ret = nvt_read_rootfsl_partition(pEmb[id].FlashType, (ulong)p_mem->p_data, p_mem->len, PartitionOffset, PartitionSize, id, &pEmb[id]);
			break;
		case EMBTYPE_RTOS:
		...
		case EMBTYPE_USER:
			...
			nvt_read_user_partition(pEmb[id].FlashType, (ulong)p_mem->p_data, p_mem->len, PartitionOffset, PartitionSize, id, &pEmb[id]);
			break;
		case EMBTYPE_USERRAW:
...
int nvt_fw_update(bool firm_upd_firm)
{
	...
	memmove((void *)CONFIG_NVT_RUNFW_SDRAM_BASE, (void *)firm_addr, firm_size);
#else
	if (firm_upd_firm) {
		unsigned long firm_addr = nvt_readl((ulong)nvt_shminfo_comm_fw_update_addr);
		unsigned long firm_size = nvt_readl((ulong)nvt_shminfo_comm_fw_update_len);
