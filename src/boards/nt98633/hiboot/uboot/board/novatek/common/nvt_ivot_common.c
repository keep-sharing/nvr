...
int nvt_fdt_init(bool reload)
{
	...
	if (reload) {
		...
		char command[128];
		#if defined(CONFIG_NVT_LINUX_SPINOR_BOOT) || defined(CONFIG_NVT_LINUX_SPINORNAND_BOOT) || (defined(CONFIG_NVT_SPI_NOR) && defined(CONFIG_NVT_LINUX_RAMDISK_SUPPORT))
		sprintf(command, "sf read 0x%lx 0x%x 0x%x", tmp_addr, CONFIG_MODELEXT_FLASH_BASE, preload_size);
		ret = run_command(command, 0);
		#elif defined(CONFIG_NVT_LINUX_SPINAND_BOOT) || (defined(CONFIG_NVT_SPI_NAND) && defined(CONFIG_NVT_LINUX_RAMDISK_SUPPORT))
...
#ifdef CONFIG_MS_NVR
#define VER_MSB S_GPIO(35)
#define VER_LSB S_GPIO(34)
...
int nvt_ivot_hw_version(void)
{
	...
	hwVer = nt98633_gpio_read(VER_LSB);
	hwVer |= (nt98633_gpio_read(VER_MSB) << 1);
	switch (hwVer) {
		case 0x00:
			...
			break;
		case 0x01:
			nvt_hw_ver =  NVT_HW_VER_5008;
			printf("** board hardware : 5008 **\n");
			break;
		case 0x10:
			...
			break;
		case 0x11:
			nvt_hw_ver =  NVT_HW_VER_1004;
			printf("** board hardware : 1004 **\n");
			break;
		default:
