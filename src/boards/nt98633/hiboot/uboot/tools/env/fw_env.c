...
int fw_printenv(int argc, char *argv[], int value_only, struct env_opts *opts)
{
...
#ifdef CONFIG_MS_NVR
            if(strstr(env, "mskey") || strstr(env, "msauthcheck") || strstr(env, "msran")) {
                continue;
            } else
#endif /* CONFIG_MS_NVR */
            {
	...
	for (i = 0; i < argc; ++i) {	/* print a subset of env variables */
		...
		char *val = NULL;
#ifdef CONFIG_MS_NVR
        if(strstr(name, "mskey") || strstr(name, "msauthcheck") || strstr(name, "msran")) {
            continue;
        }
#endif
		val = fw_getenv(name);
