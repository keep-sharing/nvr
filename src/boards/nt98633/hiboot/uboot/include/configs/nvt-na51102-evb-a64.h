 ...
 *
 * Configuration settings for the Novatek NA51102 SOC.
 *
...
#define CONFIG_SYS_NAND_BASE                            0x2F0400000
 ...
 */
#ifdef CONFIG_NOVATEK_MAC_ENET_NA51102
 ...
 */
#include "novatek/na51102_ca53_a64.h"
...
#define CONFIG_BOOTARGS_COMMON				"earlycon=nvt_serial,0x2f0290000 rootwait console=ttyS0,115200 debug_boot_weak_hash  "
/* NVT boot related setting */
#ifdef CONFIG_NVT_IVOT_SOC_FW_UPDATE_SUPPORT
	#define CONFIG_NVT_LINUX_AUTODETECT					/* Support for detect FW{SOC}A.bin/FW{SOC}T.bin automatically. (Only working on mtd device boot method) */
	#define CONFIG_NVT_BIN_CHKSUM_SUPPORT					/* This option will check rootfs/uboot checksum info. during update image flow */
		...
		#define CONFIG_NVT_LINUX_SPINAND_BOOT				/* Boot from spinand or spinor (Support FW{SOC}A.bin update all-in-one) */
		...
		#else
			/* For spi nor device */
			#define CONFIG_NVT_LINUX_SPINOR_BOOT				/* Boot from spinand or spinor (Support FW{SOC}A.bin update all-in-one) */
			#define CONFIG_BOOTARGS		CONFIG_BOOTARGS_COMMON "rootfstype=squashfs ro "
		#endif
		#define CONFIG_NVT_SQUASH_SUPPORT
			...
			#define CONFIG_NVT_LINUX_SPINAND_BOOT
				...
				#define CONFIG_BOOTARGS         CONFIG_BOOTARGS_COMMON "root=/dev/romblock8 rootfstype=squashfs ro init=/linuxrc "
			#else
				#define CONFIG_BOOTARGS         CONFIG_BOOTARGS_COMMON "root=/dev/romblock7 rootfstype=squashfs ro init=/linuxrc "
			#endif
			#define CONFIG_NVT_SQUASH_SUPPORT
		#endif
		...
		#define CONFIG_NVT_LINUX_EMMC_BOOT				/* Boot from emmc (Support FW{SOC}A.bin update all-in-one) */
		#define CONFIG_NVT_EXT4_SUPPORT
		#define CONFIG_BOOTARGS			CONFIG_BOOTARGS_COMMON "rootfstype=ext4 rw "
	#else
		#define CONFIG_NVT_LINUX_SD_BOOT				/* To handle RAW SD boot must be not compressed.*/
