...
UBOOT_DEFCONFIG := nvt-na51090_a64_emmc_defconfig
UBOOT_DTS := $(HIBOOT_DIR)/cfg/nvt-evb
ATF_DIR := $(HIBOOT_DIR)/trusted-firmware-a
ATF_IMGNAME := bl31.bin
NVT_ATF_IMG_FILE := $(ATF_DIR)/build/nvt_na51090/release/atf.bin
...
UBOOT_LD_FILE   = $(HIBOOT_DIR)/cfg/LD98633A.bin
ifeq ($(DDR_SIZE),$(DDR_SIZE_2G))
UBOOT_DDR_SIZE	= (2048*1024*1024)
else
UBOOT_DDR_SIZE	= (1024*1024*1024)
endif
UBOOT_CMDLINE	= earlycon=nvt_serial,0x2f0280000 rootwait console=ttyS0,115200 debug_boot_weak_hash  rootfstype=ext4 rw root=/dev/mmcblk1p6
...
atf:
	#@make DEBUG=0 ARCH=arm64LOG_LEVEL=40 PLAT=nvt_na51090 NEED_BL32=yes bl31 -C $(ATF_DIR) -j8
	make ARCH=aarch64 DEBUG=0 LOG_LEVEL=40 PLAT=nvt_na51090 NEED_BL32=no bl31 -C $(ATF_DIR) -j8
	@$(HIBOOT_DIR)/nvt-tools/encrypt_bin SUM $(ATF_DIR)/build/nvt_na51090/release/$(ATF_IMGNAME) 0x3a0 bl51090
	@cp -afL $(ATF_DIR)/build/nvt_na51090/release/$(ATF_IMGNAME) $(NVT_ATF_IMG_FILE)

atf-clean:
	make PLAT=nvt_na51090 clean -C $(ATF_DIR) -j8
...
hiboot: hiboot-prepare atf
	...
	@$(HIBOOT_DIR)/nvt-tools/encrypt_bin SUM $(UBOOT_DIR)/u-boot.bin 0x350 ub51090
	@$(HIBOOT_DIR)/nvt-tools/bfc c lz $(UBOOT_DIR)/u-boot.bin $(UBOOT_DIR)/u-boot.lz.bin 0 0
	@$(HIBOOT_DIR)/nvt-tools/mkboot.sh $(UBOOT_LD_FILE) $(UBOOT_DTS).dtb  $(NVT_ATF_IMG_FILE) $(UBOOT_DIR)/u-boot.lz.bin $(FIRMWARE_IMG_DIR)/uboot_$(DDR_NAME)

hiboot-clean: atf-clean
