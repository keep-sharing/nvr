...
int ddrtrn_dataeye_check_dir(unsigned int direction, unsigned int left,
...
void ddrtrn_dataeye_search_dq(unsigned int left, unsigned int right, int *target, unsigned int direction)
{
	unsigned int middle;
	unsigned int mask = 0;
	...
	if (!ddrtrn_dataeye_check_dir(direction, left, right, &mask)) { /* test ok */
		*target = (int)middle;
		return;
	}
...
/* Find DQ valid range */
void ddrtrn_dataeye_find_dq(struct ddrtrn_training_result_data *training)
{
...
int ddrtrn_dataeye_process(struct ddrtrn_training_result_data *training)
{
	int result = 0;
	unsigned int i;
