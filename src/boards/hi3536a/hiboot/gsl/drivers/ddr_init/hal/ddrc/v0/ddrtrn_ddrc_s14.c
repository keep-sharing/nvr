...
void ddrtrn_hal_axi_save_func(struct ddrtrn_hal_training_relate_reg *relate_reg)
{
	(relate_reg)->ddrc.region_attrib[0] =
		ddrtrn_reg_read(DDR_REG_BASE_AXI + DDR_AXI_REGION_ATTRIB0);
	(relate_reg)->ddrc.region_attrib[1] =
		ddrtrn_reg_read(DDR_REG_BASE_AXI + DDR_AXI_REGION_ATTRIB1);
}
...
void ddrtrn_hal_axi_switch_func(void)
{
	...
	ddrtrn_reg_write((ddrtrn_reg_read(DDR_REG_BASE_AXI + DDR_AXI_REGION_ATTRIB0) &
		AXI_REGION_ATTRIB_CH_MASK) |
		AXI_RNG_ATTR_CH_MODE | ch_start,
		DDR_REG_BASE_AXI + DDR_AXI_REGION_ATTRIB0);
	ddrtrn_reg_write((ddrtrn_reg_read(DDR_REG_BASE_AXI + DDR_AXI_REGION_ATTRIB1) &
		AXI_REGION_ATTRIB_CH_MASK) |
		AXI_RNG_ATTR_CH_MODE | ch_start,
		DDR_REG_BASE_AXI + DDR_AXI_REGION_ATTRIB1);
}
...
{
	relate_reg->ddrc.rnkvol =
		ddrtrn_reg_read(base_dmc + ddr_dmc_cfg_rnkvol(0));
}
