...
#ifdef CONFIG_MS_NVR
static int blk_arg_off_size(int argc, char *argv[], loff_t *off, loff_t *size)
{
	struct blkpart_info blkpart;
	
	if (argc >= 1 && !(str2off(argv[0], off))) {
		if ((blkparts_init() == 0) &&
			(find_blk_part(argv[0], &blkpart) == 0)) {
			*off = blkpart.offset;
			if (argc >= 2) {
...
#ifdef CONFIG_MS_NVR
	if (!str2off(argv[3], (loff_t *)&blk)) {
		if (blk_arg_off_size(argc-3, argv+3, (loff_t *)&blk, (loff_t *)&cnt) != 0) {
			printf("Error: blg_arg_off_size!\n");
			return CMD_RET_FAILURE;
		}
	} else
#endif /* CONFIG_MS_NVR */
			       ...
			       int argc, char * const argv[])
{
	...
	addr = (void *)simple_strtoul(argv[1], NULL, 16);
#ifdef CONFIG_MS_NVR
	if (!str2off(argv[2], (loff_t *)&blk)) {
		if (blk_arg_off_size(argc-2, argv+2, (loff_t *)&blk, (loff_t *)&cnt) != 0) {
			printf("Error: blg_arg_off_size!\n");
			return CMD_RET_FAILURE;
		}
	} else
...
#endif /* CONFIG_MS_NVR */
#ifdef CONFIG_MS_NVR
		printf("Error: argc:%d, valid_argc=%d!\n", argc, valid_argc);
#endif /* CONFIG_MS_NVR */
		return CMD_RET_USAGE;
	}
#ifdef CONFIG_MS_NVR
	if (valid_argc == 2) {
		if (blk_arg_off_size(argc-1, argv+1, (loff_t *)&blk, (loff_t *)&cnt) != 0) {
			printf("Error: blg_arg_off_size!\n");
			return CMD_RET_FAILURE;
		}
	} else
