...
#ifdef CONFIG_OT_MIPI_TX_SUPPORT
#include "mipi_tx.h"
#endif
...
static void do_start_hdmi(unsigned int intftype, unsigned int sync)
{
    ...
    if (intftype & OT_VO_INTF_HDMI) {
        hdmi_display(OT_HDMI_ID_0, sync, OT_HDMI_VIDEO_MODE_YCBCR444, OT_HDMI_VIDEO_MODE_YCBCR444);
    }

    if (intftype & OT_VO_INTF_HDMI1) {
        hdmi_display(OT_HDMI_ID_1, sync, OT_HDMI_VIDEO_MODE_YCBCR444, OT_HDMI_VIDEO_MODE_YCBCR444);
    }
#else
    if (intftype & OT_VO_INTF_HDMI) {
...
{
    unsigned int intftype = vo_get_interface_type(dev);
#ifdef HDMI_SUPPORT_DUAL_CHANNEL
    if (intftype & OT_VO_INTF_HDMI) {
        hdmi_stop(OT_HDMI_ID_0);
    }

    if (intftype & OT_VO_INTF_HDMI1) {
        hdmi_stop(OT_HDMI_ID_1);
    }
#else
    if (intftype & OT_VO_INTF_HDMI) {
        hdmi_stop();
