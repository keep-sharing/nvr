...
#define DOWNLOAD_ADDR      (0x42000000)
...
static int update_write_nand(char *part_name, ulong addr, ulong len)
{
    ...
    if (!strcmp(part_name, "fs1") ||
        ...
        sprintf(cmd_write, "mmc write %d 0x%lx %s", dev, addr, part_name);
    }
...
static int image_verify(UPFW_HEADER_T *upfw)
{
    ...
    for (i = 0; i < UPFW_MAX; i++) {
        if (i == UPFW_BLD) {
            ...
            if (plat_version != 77) { //3516 uboot_version is 4.X.X.X
...
int do_download(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
    ...
    for (i = 0; i < UPFW_MAX; i++) {
        if (MS_DDR_VERSION == 2) {//2G ddr,跳过BLD2,KER2
            if((i == UPFW_BLD2) || (i == UPFW_KER2)) {
                continue;
            }
        } else if (MS_DDR_VERSION == 3) {//4g ddr,跳过BLD,KER
            if((i == UPFW_BLD) || (i == UPFW_KER)) {
                continue;
            }
        }
