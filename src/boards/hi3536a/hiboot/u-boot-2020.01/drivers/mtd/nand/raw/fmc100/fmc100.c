...
static void set_addr_reg(struct fmc_host *host)
{
	...
	chip = host->chip;
	pages_per_block_shift = (unsigned char)(chip->phys_erase_shift - chip->page_shift);
	block_num = host->addr_value[1] >> pages_per_block_shift;
	block_num_h = block_num >> REG_CNT_HIGH_BLOCK_NUM_SHIFT;
	reg = (unsigned int)(fmc_addrh_set(block_num_h));
...
static void pageprog_fmc_op(const struct fmc_host*  host, const struct fmc_spi* spi)
{
	unsigned int reg;
...
static int fmc100_check_ip_user(unsigned char *fmc_ip)
{
	if (*fmc_ip) {
...
static void fmc100_send_cmd_pageprog(struct fmc_host *host)
{
	...
	if (fmc100_check_ip_user(fmc_ip) != 0)
		return;
...
static void erase_fmc_op(const struct fmc_host* host, const struct fmc_spi* spi)
{
	unsigned int reg;
...
static void fmc100_send_cmd_erase(const struct fmc_host *host)
{
	const struct fmc_spi *spi = host->spi;
	unsigned char *fmc_ip = get_fmc_ip();
	int ret;
...
static void fmc100_send_cmd_status(const struct fmc_host* host)
{
	unsigned int status;
...
static void fmc100_send_cmd_reset(const struct fmc_host* host)
{
	unsigned int reg;
				...
				int column, int page_addr)
{
	...
	case NAND_CMD_READ0:
		if (command == NAND_CMD_READ0)
			host->cmd_op.l_cmd = command & 0xff;
		host->addr_value[1] = (unsigned int)page_addr;
	...
	case NAND_CMD_ERASE1:
		host->cmd_op.l_cmd = command & 0xff;
		host->addr_value[0] = (unsigned int)page_addr;
...
static int fmc100_get_block_reg(struct fmc_host* const host,
			const struct nand_config_info* info, unsigned int* const block_val)
{
...
static void fmc100_set_fmc_cfg_reg(struct mtd_info* const mtd,
				const struct nand_config_info* type_info)
{
	struct nand_chip *chip = NULL;
...
static int host_data_init(struct fmc_host *host)
{
