...
static int write_bp_area_check(struct fmc_host* const host, u_int to, size_t len)
{
#ifdef CONFIG_SPI_BLOCK_PROTECT
	if (host->level) {
...
static int fmc100_erase_parm_check(const struct spi_flash* spiflash,
					u32 offset,
					size_t length,
...
struct spi_flash *fmc100_spi_nor_scan(struct fmc_host *host)
{
	...
	if (fmc_spi_nor_probe(spi_nor_info, spi) <= 0) {
...
static void fmc100_set_host_addr_mode(const struct fmc_host* host, int enable)
{
...
void fmc_check_spi_dtr_support(struct fmc_spi *spi, const u_char *ids, int len)
{
