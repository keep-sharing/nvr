...
*/
unsigned char spi_general_get_flash_register(const struct fmc_spi * spi, u_char cmd)
{
...
*/
static int spi_general_wait_ready(const struct fmc_spi *spi)
{
	unsigned char status;
	...
	do {
		...
		if ((status & SPI_NOR_SR_WIP_MASK) == 0)
			return 0;
...
*/
static int spi_general_write_enable(const struct fmc_spi *spi)
{
	unsigned char status;
	unsigned int reg;
...
static void reset_pin_enable_fmc_op(const struct fmc_host* host,
				const struct fmc_spi* spi)
{
	unsigned int regval;
