...
{
    char *chipinfo = env_get("chipinfo");
    if (chipinfo) {
...
{
    env_set("jpeg_addr", "0x40000000");
    env_set("jpeg_size", "0x80000");//512KB
    //env_set("read_jpeg_size", "0x400");//1 block=512 Byte
    env_set("jpeg_emar_buf", "0x40080000");//解码JPEG过程中使用到的内存地址

    env_set("vobuf", "0x41800000");//解码后的地址
    run_command("saveenv", 0);
    run_command("mmc read 0 0x40000000 logo", 0);
    run_command("decjpg 0", 0); //解码JPEG到内存

    run_command("setvobg 0 0", 0);
    run_command("startvo 0 4096 23", 0);//HDMI 1
