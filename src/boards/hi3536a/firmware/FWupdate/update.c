...
static const char *short_options = "?uadelB:S:N:C:D:E:W:";
static struct option long_options[] = {
    ...
    {"demotion_limit", no_argument, 0, 'd'},
    {"oem", no_argument, 0, 'e'},
...
int main(int argc, char* argv[])
{
    ...
    if (demotion_limit) {
           int fd = open(fw_name, O_RDONLY | O_BINARY);
           if (fd >= 0) {
               ...
               if (read(fd, &headInfo, IMG_HEADER_SIZE) == IMG_HEADER_SIZE) {
                   struct tm tm_img;
                   time_t t_img;
                   sscanf(headInfo.img_date, "%4d-%2d-%2d %2d:%2d:%2d",
                          &tm_img.tm_year, &tm_img.tm_mon, &tm_img.tm_mday, &tm_img.tm_hour, &tm_img.tm_min, &tm_img.tm_sec);
                   tm_img.tm_year -= 1900;
                   tm_img.tm_mon -= 1;
                   tm_img.tm_isdst = -1;
                   t_img = mktime(&tm_img);
                   ...
                   struct tm tm_fac;
                   time_t t_fac;
                   char *fac_date = hs_getenv("factory_date");
                   char *fac_version = hs_getenv("factory_version");
                   debug("image_date:%s, image_version:%s, factory_date:%s, factory_version:%s\n",
                         headInfo.img_date, headInfo.img_version, fac_date, fac_version);
                   if (fac_date) {
                       sscanf(fac_date, "%4d-%2d-%2d %2d:%2d:%2d",
                              &tm_fac.tm_year, &tm_fac.tm_mon, &tm_fac.tm_mday, &tm_fac.tm_hour, &tm_fac.tm_min, &tm_fac.tm_sec);
                       tm_fac.tm_year -= 1900;
                       tm_fac.tm_mon -= 1;
                       tm_fac.tm_isdst = -1;
                       t_fac = mktime(&tm_img);
               ...
               if (err_no != UPFW_SUCCESS) {
                   snprintf(errbuf, sizeof(errbuf), UPFW_ERR_ERRSTR_FMT, "factory version limit!");
