...
const struct spi_flash_params spi_flash_params_table[] = {
	...
	{"GD25Q32",       0xc84016, 0x0,       64 * 1024,    64,       RD_FULL, SPI_FLASH_BUSWIDTH_QUAD_TYPE4},
...
static void nvt_parse_frequency(u32 *freq)
{
	...
	*freq = 48000000;
	...
	sprintf(path,"/nor@%x",CONFIG_SYS_NAND_BASE);
...
{
	u8 wip_status, qeb_status;
	u8 v_status[2];
	u32 data;
	int ret;

	ret =  spinor_read_status(info, NAND_SPI_NOR_STS_RDSR_1, &wip_status);
	if (ret < 0) {
		return ret;
	}
	ret =  spinor_read_status(info, NAND_SPI_NOR_STS_RDSR_2, &qeb_status);
	...
	if (qeb_status & STATUS_QEB_WINSPAN) {
		...
		v_status[0] = wip_status;
		v_status[1] = data;
		ret = spinor_write_status_array(info, v_status, 2);
		if (ret < 0) {
			return ret;
		}
...
{
	uint32_t reg, freq;
#ifndef CONFIG_NVT_IVOT_SOC_COMMON_UTILS
	u32 *cell = NULL;
	char path[20] = {0};
	int len;
	...
	reg = readl(IOADDR_PAD_REG_BASE + 0x00);
	reg &= ~0xFFFC00;
	reg |= 0xAAA800;
	writel(reg, IOADDR_PAD_REG_BASE + 0x00);
	...
	info->flash_type = NANDCTRL_SPI_NOR_TYPE;
		...
		unsigned int max_hz, unsigned int spi_mode)
{
	...
	info->data_buff = calloc(info->flash_info->page_size + CACHE_LINE_SIZE, sizeof(unsigned char));
