 ...
 * na51068:
 *    NOVATEK's NA51068 chip. This configuration uses an AXI master/DMA bus, an
...
static int eqos_start(struct eth_device *eth_dev, bd_t *bis)
{
	...
	if (!eqos->phy) {
		ret = -1;
		error("no phy is connected");
		goto err;
	}

	if (eqos->phy_intf == PHY_INTERFACE_MODE_NONE) {
		ret = -1;
		error("no phy pinmux");
		goto err;
	}
	...
	if (ret) {
		error("EQOS_DMA_MODE_SWR stuck (maybe pinmux config error or phy not exist)");
		goto err;
	}
...
static void mac0_clk_en(phy_interface_t phy_intf)
{
	...
	if (phy_intf == PHY_INTERFACE_MODE_NONE) return;
...
static void extphy0_clk_en(phy_interface_t phy_intf, int phy_clk, int ref_clk_out)
{
	UINT reg;
	...
	case PHY_INTERFACE_MODE_RGMII:
		...
		// reset external phy
		reg = readl(IOADDR_GPIO2_REG_BASE + 0x08);	// set to output
		reg |= 1<<21;
		writel(reg, IOADDR_GPIO2_REG_BASE + 0x08);
		reg = readl(IOADDR_GPIO2_REG_BASE + 0x14);	// drive LOW
		reg |= 1<<21;
		writel(reg, IOADDR_GPIO2_REG_BASE + 0x14);
		mdelay(10);	// assert low at least 10ms
		reg = readl(IOADDR_GPIO2_REG_BASE + 0x10);	// drive HIGH
		reg |= 1<<21;
		writel(reg, IOADDR_GPIO2_REG_BASE + 0x10);
...
static void extphy1_clk_en(phy_interface_t phy_intf, int phy_clk, int ref_clk_out)
{
	UINT reg;
	...
	case PHY_INTERFACE_MODE_RGMII:
		...
		if (phy_clk > 0) {
			reg = readl(IOADDR_CG_REG_BASE + 0xE4);
			reg |= (1<<23);// release ETH1 ext phy clk output
			writel(reg, IOADDR_CG_REG_BASE + 0xE4);

			// enable pinmux
			reg = readl(IOADDR_TOP_REG_BASE + 0x50);
			reg &= ~(0x7<<0);
			if (phy_intf == PHY_INTERFACE_MODE_RGMII) {
				reg |= (6<<0);
			} else if (phy_intf == PHY_INTERFACE_MODE_RMII) {
			...
			writel(reg, IOADDR_TOP_REG_BASE + 0x50);
		}

		// reset external phy
		reg = readl(IOADDR_GPIO1_REG_BASE + 0x08);	// set to output
		reg |= 1<<17;
		writel(reg, IOADDR_GPIO1_REG_BASE + 0x08);
		reg = readl(IOADDR_GPIO1_REG_BASE + 0x14);	// drive LOW
		reg |= 1<<17;
		writel(reg, IOADDR_GPIO1_REG_BASE + 0x14);
		mdelay(10);	// assert low at least 10ms
		reg = readl(IOADDR_GPIO1_REG_BASE + 0x10);	// drive HIGH
		reg |= 1<<17;
		writel(reg, IOADDR_GPIO1_REG_BASE + 0x10);
		mdelay(50);	// wait 50ms before access phy register
		break;
	case PHY_INTERFACE_MODE_MII:
		reg = readl(IOADDR_ETH1_REG_BASE + 0x3004);
...
static void mac0_pinmux_en(phy_interface_t phy_intf)
{
	UINT reg;
	...
	case PHY_INTERFACE_MODE_RMII:
		...
		reg &= ~(0xF<<4);
		writel(reg | (0x4<<4), IOADDR_TOP_REG_BASE + 0xC8);
		reg = readl(IOADDR_TOP_REG_BASE + 0x70);
		reg &= ~(0xFFF0F);
		writel(reg | 0x00222202, IOADDR_TOP_REG_BASE + 0x70);
		reg = readl(IOADDR_TOP_REG_BASE + 0x74);
		reg &= ~(0xFF00FFF0);
		writel(reg | 0x22002220, IOADDR_TOP_REG_BASE + 0x74);

		//Pad pullup
		reg = readl(IOADDR_PAD_REG_BASE + 0x40);
		reg &= ~(0xF000000C);
		writel(reg | 0xA0000008, IOADDR_PAD_REG_BASE + 0x40);
		break;
	case PHY_INTERFACE_MODE_RGMII:
		// set pad driving
		reg = readl(IOADDR_PAD_REG_BASE + 0xC0);
		...
		reg = readl(IOADDR_TOP_REG_BASE + 0xC8);
		reg &= ~(0xF<<4);
		writel(reg | (0x1<<4), IOADDR_TOP_REG_BASE + 0xC8);
		reg = readl(IOADDR_TOP_REG_BASE + 0x70);
		reg &= ~(0xFFFFFF0F);
		writel(reg | 0x11111101, IOADDR_TOP_REG_BASE + 0x70);
		reg = readl(IOADDR_TOP_REG_BASE + 0x74);
		reg &= ~(0xFFFFFFFF);
		writel(reg | 0x11111111, IOADDR_TOP_REG_BASE + 0x74);
		...
		writel(reg | 0xA0000008, IOADDR_PAD_REG_BASE + 0x40);
		break;
	case PHY_INTERFACE_MODE_MII: // embedded phy (MII)
		reg = readl(IOADDR_TOP_REG_BASE + 0xC8);
		reg &= ~(0xF<<4);
...
static void mac1_pinmux_en(phy_interface_t phy_intf)
{
	UINT reg;
	...
	case PHY_INTERFACE_MODE_RMII:
		...
		reg &= ~((0x3<<2) | (0x3<<0));
		writel(reg | 0x01, IOADDR_PAD_REG_BASE + 0xA0);	// PHYCLK (25M) 8 mA, others 4 mA
		reg = readl(IOADDR_PAD_REG_BASE + 0xA4);
		reg &= ~((0x3<<4) | (0x3<<2) | (0x3<<0));
		writel(reg, IOADDR_PAD_REG_BASE + 0xA4);	// TXCTL, TXD 4 mA

		// setup pinmux
		reg = readl(IOADDR_TOP_REG_BASE + 0x50);
		reg &= ~(0xFFFF0F);
		writel(reg | 0x222202, IOADDR_TOP_REG_BASE + 0x50);
		reg = readl(IOADDR_TOP_REG_BASE + 0x54);
		reg &= ~(0xFF00FFF0);
		writel(reg | 0x22002220, IOADDR_TOP_REG_BASE + 0x54);

		reg = readl(IOADDR_TOP_REG_BASE + 0xC8);
		reg &= ~(0xF<<8);
		writel(reg | (0x4<<8), IOADDR_TOP_REG_BASE + 0xC8);
		break;
	case PHY_INTERFACE_MODE_RGMII:
		// set pad driving
		reg = readl(IOADDR_PAD_REG_BASE + 0xA0);
		// PHYCLK (25M), TXCLK 8 mA
		reg &= ~((0x3<<16) | (0x3<<0));
		reg |= (0x1<<16) | (0x1<<0);
		writel(reg, IOADDR_PAD_REG_BASE + 0xA0);
		reg = readl(IOADDR_PAD_REG_BASE + 0xA4);
		// TXCLK, TXD 8 mA
		reg &= ~((0x3<<8) | (0x3<<6) | (0x3<<4) | (0x3<<2) | (0x3<<0));
		reg |= (0x1<<8) | (0x1<<6) | (0x1<<4) | (0x1<<2) | (0x1<<0);
		writel(reg, IOADDR_PAD_REG_BASE + 0xA4);

		// setup pinmux
		reg = readl(IOADDR_TOP_REG_BASE + 0x50);
		reg &= ~(0xFFFFFF0F);
		writel(reg | 0x66666606, IOADDR_TOP_REG_BASE + 0x50);
		reg = readl(IOADDR_TOP_REG_BASE + 0x54);
		reg &= ~(0xFFFFFF0F);
		writel(reg | 0x66666606, IOADDR_TOP_REG_BASE + 0x54);

		reg = readl(IOADDR_TOP_REG_BASE + 0xC8);
		reg &= ~(0xF<<8);
		writel(reg | (0x1<<8), IOADDR_TOP_REG_BASE + 0xC8);
		break;
	case PHY_INTERFACE_MODE_MII: // embedded phy (MII)
		reg = readl(IOADDR_TOP_REG_BASE + 0xC8);
		reg &= ~(0xF<<8);
...
struct NVT_PLAT_INFO {
	UINT	base_pa;		// base physical address
	char	*dtb_name;
	void	(*mac_pinmux_en)(phy_interface_t phy_intf);
	void	(*mac_clk_en)(phy_interface_t phy_intf);
	void	(*extphy_clk_en)(phy_interface_t phy_intf, int phy_clk, int ref_clk_out);
...
static const struct NVT_PLAT_INFO v_nvt_plat_info[2] = {
	...
	// MAC1 (0xFCD0_0000)
	{
		...
		PIN_ETH_CFG_PHY_ROUTE_ETH1,

		1*32+17,			// gpio reset
	},
};

static int eth_parse_phy_intf(int base_addr, struct eqos_priv *eqos)
{
	...
	sprintf(path,"/top@%x/misc",IOADDR_TOP_REG_BASE);
	...
	if (phy_intf & v_nvt_plat_info[ETH_QOS_INDEX].pinmux_rmii) {
		...
		printf("%s: pinmux NOT detected 0x%x\r\n", __func__, phy_intf);
		ETH_PHY_INTF = PHY_INTERFACE_MODE_NONE;
	}
	v_nvt_plat_info[ETH_QOS_INDEX].mac_pinmux_en(ETH_PHY_INTF);
	v_nvt_plat_info[ETH_QOS_INDEX].mac_clk_en(ETH_PHY_INTF);	// also assert async reset
	v_nvt_plat_info[ETH_QOS_INDEX].extphy_clk_en(ETH_PHY_INTF, phy_clk, ref_clk_out);
	eqos->phy_intf = ETH_PHY_INTF;
#if 0
	if (phy_intf & 0x1) {
...
#ifdef CONFIG_MS_NVR
static int eqos_set_mac_address(struct eth_device *dev)
{
	ulong fdt_addr = (ulong)nvt_fdt_buffer;//nvt_readl((ulong)nvt_shminfo_boot_fdt_addr);
	UINT ETH_QOS_INDEX;	// index to MAC (0: MAC0, 1: MAC1);
    uchar mac[6];
    char *s, *e;
	int nodeoffset;
    ...
    rc = fdt_setprop((void*)fdt_addr, nodeoffset, "mac-address", mac, sizeof(mac));
	if (rc < 0) {
		printf("set fdt [%s] mac address failed\n", dev->name);
		return -1;
	}
...
static int eqos_initialize(bd_t *bis, int base_addr)
{
	struct eth_device *dev;
	...
	ret = eth_parse_phy_intf(base_addr, eqos);
	if (ret < 0) return ret;
...
#ifdef CONFIG_MS_NVR
static int eqos_custom_fdtinit(void)
{
    char path[20] = {0};
    int ret = 0;
    int nodeoffset;
    ...
    if (nvt_ivot_hw_version() == NVT_HW_VER_1009) {
        sprintf(path,"/top@%x/eth",IOADDR_TOP_REG_BASE);
        nodeoffset = fdt_path_offset((const void*)fdt_addr, path);
        if (nodeoffset < 0) {
            printf("%s(%d) nodeoffset < 0\n",__func__, __LINE__);
        ...
        //pinmux-> eht0:rgmii 1Gbps eth1: rmii 100Mbps
        val = PIN_ETH_CFG_RGMII_1ST_PINMUX | PIN_ETH_CFG_RMII_2ND_PINMUX | PIN_ETH_CFG_EXTPHYCLK_1ST_PINMUX;
        ret = fdt_setprop_u32((void*)fdt_addr, nodeoffset, "pinmux", val);
        if (ret < 0) {
            printf("%s(%d) fdt_setprop_u32 pinmux failed\n",__func__, __LINE__);
            return -1;
        }
        //eth1 setting fixed-link
        sprintf(path,"/%s", v_nvt_plat_info[1].dtb_name);
        nodeoffset = fdt_path_offset((const void*)fdt_addr, path);
        if (nodeoffset < 0) {
            printf("%s(%d) nodeoffset < 0\n",__func__, __LINE__);
            printf("%s: path %s not found\n", __func__, path);
        ...
        val = 0; // disable sp-clk
        ret = fdt_setprop_u32((void*)fdt_addr, nodeoffset, "sp-clk", val);
        if (ret < 0) {
            printf("%s(%d) fdt_setprop_u32 sp-clk failed\n",__func__, __LINE__);
            return -1;
        }

        val = 1; // enbale ref-clk-out
        ret = fdt_setprop_u32((void*)fdt_addr, nodeoffset, "ref-clk-out", val);
        if (ret < 0) {
            printf("%s(%d) fdt_setprop_u32 ref-clk-out failed\n",__func__, __LINE__);
            return -1;
        }

        nodeoffset = fdt_subnode_offset((const void*)fdt_addr, nodeoffset, "fixed-link");
        if (nodeoffset < 0) {
            printf("%s(%d) subnodeoffset < 0\n",__func__, __LINE__);
            printf("%s: path %s not found\n", __func__, "fixed-link");
            return -1;
        }

        val = 100; // rmii fixed-link speed 100Mbps
        ret = fdt_setprop_u32((void*)fdt_addr, nodeoffset, "speed", val);
        if (ret < 0) {
            printf("%s(%d) fdt_setprop_u32 speed failed\n",__func__, __LINE__);
            return -1;
        }
...
int na51068_eth_initialize(bd_t *bis)
{
	int ret = 0;
	u32 i;
	char path[20] = {0};
