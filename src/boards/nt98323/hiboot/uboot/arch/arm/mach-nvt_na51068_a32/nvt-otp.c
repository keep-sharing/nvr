...
static const UINT32   cpu_power_leakage[LEAKAGE_COUNT] = {
		0,
		54,
		54,
		54,
...
*/
BOOL extract_trim_valid(UINT32 code, UINT32 *pData)
{
    UINT32 i;
    UINT32 ret = 0;
    BOOL is_found = FALSE;
...
/*
    ...
    @return read half-word efuse data
        - @b  UINT32: PWM dufy
*/
#define CPU_DEFAULT_DUTY 62
#define CORE_DEFAULT_DUTY 50
UINT32 extract_trim_to_pwm_duty(PWM_POWER_SET power_set, UINT32 data, UINT32 freq)
{
	UINT32 duty = 0;

	if(power_set == PWM1_CPU_POWER) {
		if(data == 0x0) {
			duty = CPU_DEFAULT_DUTY;
		} else if(data < LEAKAGE_COUNT){	// Table loop uo
			duty = cpu_power_leakage[data];
		} else {	//leakage >= 21 =>
			duty = 62;
		}
		if(freq > 1200 && duty >= 40) {
			duty -= 40;
		}
	} else if(power_set == PWM0_CORE_POWER){
		if(data == 0x0) {
