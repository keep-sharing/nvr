 ...
 */
/* logo base */
#define CONFIG_LOGO_HDMI_FRAME_SDRAM_BASE	(CONFIG_NVT_RUNFW_SDRAM_BASE - SZ_1M * 1)/*放hdmi logo jpg解码的YUV数据位置*/
#define CONFIG_LOGO_CVBS_FRAME_SDRAM_BASE	(CONFIG_LOGO_HDMI_FRAME_SDRAM_BASE - SZ_1M * 5)/*放cvbs logo jpg解码的YUV数据位置*/

#define FRAME_BUFFER_BASE	CONFIG_LOGO_CVBS_FRAME_SDRAM_BASE//0x10800000   //TBD, configurable
#define FRAME_BUFFER_SIZE	(1 << 20)    //1M, configurable

static lcd_output_t g_output[1] = {OUTPUT_PAL};   //configurable  OUTPUT_PAL  OUTPUT_NTSC
static lcd_vin_t    g_vin[1] = {VIN_PAL};         //configurable  VIN_PAL  VIN_NTSC
static input_fmt_t  g_infmt[1] = {INPUT_FMT_YUV422 /* INPUT_FMT_RGB565 */};    //configurable

#define PATGEN_ENABLE   0   /* 1 for enable pattern generation mabe 1 by ceg*/
#define VERSION			"1.0"
...
static u32 diplay_pll[1] = {297000}; /* khz */
#ifndef ARRAY_SIZE
...
#define LCD210_BASE		CONFIG_LCDC210_BASE
#define TVE100_BASE		CONFIG_FTTVE100_BASE
#define LCD210_PLL      2   /*0: PLL9, 2: PLL11, default PLL11  开机画面正常设置：hdmi vga:2   cvbs:0  */ 
                            /* 备注使用 ‘0’ 的时候： vga开机画面最后会出现蓝屏, hdmi cvbs 画面正常
							   备注使用 ‘2’ 的时候： cvbs开机画面会在nv_boot 重启中显示开机画面, hdmi vga 画面正常   
							 */
...
static void ftpmu010_write_reg(int lcd_fd, unsigned int offset, unsigned int val, unsigned int mask)
{
...
static void platform_pmu_clock_gate(int on_off)
{
	...
	if (on_off == 1) {
    	...
    	bit = (0x0 << 14);
    	mask = (0x1 << 14);
    	ftpmu010_write_reg(lcd_fd, 0x60, bit, mask);
		/* gating clock - HClkOff. 0: Release, 1: Gating. TVE */
		bit = (0x0 << 13);
		mask = (0x1 << 13);
		ftpmu010_write_reg(lcd_fd, 0x68, bit, mask);

		/* gating clock - PClk0Off. 0: Release, 1: Gating. lcd210p */
    	bit = 0x0 << 7;
    	mask = 0x1 << 7;
    	ftpmu010_write_reg(lcd_fd, 0x70, bit, mask);

    	/* turn on DAC */
		/*ftpmu010_write_reg(lcd_fd, 0xFC, 0x0, 0xF);*/
		iowrite32(0xF0C, TVE100_BASE + 0x44);
    } else {
    	/* gating clock - AClkOff0. 0: Release, 1: Gating. lcd210a */
    	bit = (0x1 << 14);
    	mask = (0x1 << 14);
		...
		iowrite32(0xF0F, TVE100_BASE + 0x44);
    }
}
...
static unsigned int platform_pmu_calculate_clk(u32 pixclk)
{
	...
	if (LCD210_PLL == 2) {
		 ...
		 * [9:8]: lcd200 clock select. 0:  pll9(DIS0_CK), 1: pll10(DIS1_CK), 2: pll11(DIS2_CK)
		 */
		pll = diplay_pll[2];
		clk_src = 2; /* pll11 */
		div = ((pll / pixclk)-1) & 0x3F;
		printk("LCD210_1: chooses PLL11:%d with div=%d\n", pll, div+1);
	} else if (LCD210_PLL == 1) {
		pll = diplay_pll[1];
		clk_src = 1; /* pll10 */
		div = ((pll / pixclk)-1) & 0x3F;
		printk("LCD210_1: chooses PLL10:%d with div=%d\n", pll, div+1);
	} else {
		pll = diplay_pll[0];
		clk_src = 0; /* pll9 */
		div = ((pll / pixclk)-1) & 0x3F;
		printk("LCD210_1: chooses PLL9:%d with div=%d\n", pll, div+1);
	}
#else
	...
	/* [9:8]: lcdc210_1 pixclk_cntp pvalue */
	ftpmu010_write_reg(lcd_fd, 0x174, (div << 10) | (clk_src << 8), (0x3F << 10) | (0x3 << 8));
...
static int drv_video_init(int output)
{
    ...
    if(code == -33) {
        ...
        if(is_found) {
			...
			if ((((dac_trim & 0x1F) | 0xE0) >= 0xED) && (((dac_trim & 0x1F) | 0xE0) < 0xF4)) {	
				//TV_DAC_PD_CTRL_OFS 0x44
				reg_temp = ioread32(TVE100_BASE + 0x44);
				reg_temp &= ~(0x1F << 4);
				reg_temp |= ((dac_trim & 0x1F)  << 4);
...
int lcd210_init(lcd_output_t output, lcd_vin_t vin, input_fmt_t	infmt, u32 frame_base)
{
	...
	if (set_infmt(infmt) < 0)
		return -1;
	set_framebase(FRAME_BUFFER_BASE);
	enable_controller(output, infmt);

	return 0;
}

void flcd210_main(int cvbs)
{
    ...
    ret = lcd210_init(vin, output, infmt, FRAME_BUFFER_BASE);
	if (!ret) {
		/* enable cvbs */
