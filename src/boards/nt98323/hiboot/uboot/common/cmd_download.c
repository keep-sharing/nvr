...
//    printf("cmd_erase = %s\n", cmd_erase);
    ...
    sprintf(cmd_erase, "sf erase 0 0x60000");
//    printf("cmd_erase = %s\n", cmd_erase);
    rc = run_command(cmd_erase , 0);
    if(rc < 0)
    {
...
static int update_write_nand(char *part_name, ulong addr, ulong len)
{
    ...
    sprintf(cmd_erase, "nand erase.part %s", part_name);
//    printf("cmd_erase = %s\n", cmd_erase);
    rc = run_command(cmd_erase , 0);
    if(rc != 0)
    {
	...
	if(!strcmp(part_name,"ker1"))
		{
			...
			if(rc < 0)
        	{    
    	    	printf(">>>>>update ker2 failed !<<<<<\n");
            	return -1;
       		}
			
			
			printf(">>>>>update ker2 success !<<<<<\n\n");
		}
		...
		if(!strcmp(part_name,"fs1"))
		{
			...
			if(rc < 0)
        	{    
    	    	printf(">>>>>update fs2 failed !<<<<<\n");
            	return -1;
       		}
			
			
			printf(">>>>>update fs2 success !<<<<<\n\n");
		}
