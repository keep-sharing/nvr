...
#ifdef CONFIG_MS_NVR
#define STAT_GREEN_LED  GPIO_1(10)
#define BUZZ_CTL        GPIO_0(30)
#define POWER_GP0       GPIO_0(17)
#define POWER_GP1       GPIO_0(18)
#define RESET_KEY       GPIO_1(8)
...
{
    return run_command("bootlogo hdmi", 0);
}

static int show_chipinfo(void)
{
    char *chipinfo = getenv("chipinfo");
    if (chipinfo) {
        printf("Read value from env %s\n", chipinfo);
    } else {
        printf("chipinfo not found\n");
    }
    return 0;
}
...
{
    gpio_direction_output(POWER_GP1, 1);
    
    gpio_direction_output(POWER_GP0, 1);
    udelay(50000);
    gpio_direction_output(POWER_GP0, 0);
    gpio_direction_input(POWER_GP0);
    gpio_direction_output(POWER_GP1, 0);
    gpio_direction_output(BUZZ_CTL, BUZZ_ON);
    udelay(200000);
    gpio_direction_output(BUZZ_CTL, BUZZ_OFF);
	gpio_direction_output(STAT_GREEN_LED, LED_ON);
    return 0;
}
static void m_delay(int n)
{
    while(n --) {
        udelay(1000);
    }
...
{
    run_command("nand erase.part cfg", 0);
    printf("restore factory done ... \n");
    buzz(20);
}

static int switch_setting(void)
{
	char *bootargs;
	char *bootcmd;
	char *p;
...
{
    unsigned char i = 0;
    gpio_direction_input(RESET_KEY);
    if(gpio_get_value(RESET_KEY) != RESET_KEY_PRESSED) {
        return 0;
    } else {
