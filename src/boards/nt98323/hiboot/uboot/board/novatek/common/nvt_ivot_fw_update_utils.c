...
int get_part(const char *partname, loff_t *off, loff_t *maxsize)
{
	...
	char cmdline[CONFIG_CMDLINE_SIZE] = {0}, nvtemmcpart[32] = {0};
...
static unsigned short check_flash_flag = 1;
...
#ifdef CONFIG_NVT_SPI_NOR_NAND
        if ((addr < flash_offset) && check_flash_flag) { /* nor swap to nand */
            check_flash_flag = 0; /* not check again */
        }

        if (!check_flash_flag) {
            pemb[part_id].FlashType = FLASHTYPE_NAND;
        }
		flash_offset = addr;
#endif
...
int nvt_getfdt_rootfs_mtd_num(ulong fdt_addr, unsigned int *mtd_num, uint32_t *ro_attr)
{
	...
	sprintf(path, "/mmc@%x", IOADDR_SDIO3_REG_BASE);
	nodeoffset = fdt_path_offset((void*)fdt_addr, path);
	#elif defined(CONFIG_NVT_SPI_NOR) || defined(CONFIG_NVT_SPI_NOR_NAND) //zbing add defined(CONFIG_NVT_SPI_NOR_NAND)
	nodeoffset = fdt_path_offset((void*)fdt_addr, "/nor");
	#else
	nodeoffset = fdt_path_offset((void*)fdt_addr, "/nand");
	#endif
