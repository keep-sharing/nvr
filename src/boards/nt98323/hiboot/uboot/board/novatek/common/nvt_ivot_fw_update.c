...
static int nvt_update_partitions(unsigned short	FlashType, u64 addr, u64 size, u64 part_off, u64 PartitionSize)
{
	char command[128];
	u32 align_size = 0;
...
static int nvt_read_partitions(unsigned short FlashType, u64 addr, u64 size, u64 part_off, unsigned int pat_id, unsigned short embtype)
{
	char command[128];
            ...
            case FLASHTYPE_NAND:
                {
                    ...
                    if(second_part_size > 0)
                    {
                        ...
                        run_command(command, 0);
                        memcpy((void*)((unsigned int)addr+first_part_size), (void*)buf, second_part_size);
                    }
                    free(buf);
                }
                break;
...
static int nvt_update_fs_partition(u64 addr, u64 size, u64 part_off, u64 part_size, EMB_PARTITION *pEmb)
{
	char command[128];
...
#if defined(CONFIG_NVT_LINUX_RAMDISK_BOOT)
	#if defined(CONFIG_NVT_SPI_NAND) || defined(CONFIG_NVT_SPI_NOR_NAND)
		size = ALIGN_CEIL(size, nand_get_block_size());
...
static int nvt_update_user_partition(u64 addr, u64 size, u64 part_off, u64 part_size, EMB_PARTITION *pEmb)
{
	char command[128];
	...
	if (size > part_size) {
		printf("%s partition size is too small (0x%llx > 0x%llx) %s\r\n", ANSI_COLOR_RED, size, part_size, ANSI_COLOR_RESET);
		return -1;
	}
	...
	sprintf(command, "nand write.trimffs 0x%llx rootfs%u 0x%llx", addr, pEmb->OrderIdx, size);
	run_command(command, 0);
	#else
	sprintf(command, "sf write 0x%llx 0x%llx 0x%llx", addr, part_off, size);
	run_command(command, 0);
	...
	printf("Update: %s %s\n", __func__, "SquashFS");
	#if defined(CONFIG_NVT_LINUX_SPINAND_BOOT)
	sprintf(command, "nand write 0x%llx rootfs%u 0x%llx", addr, pEmb->OrderIdx, size);
	run_command(command, 0);
	#else
	sprintf(command, "sf write 0x%llx 0x%llx 0x%llx", addr, part_off, size);
	run_command(command, 0);
	...
	sprintf(command, "nand write.trimffs 0x%llx user%u_%d 0x%llx", addr, GET_USER_PART_NUM(pEmb->EmbType), pEmb->OrderIdx, size);
	printf("%s\n", command);
	run_command(command, 0);
#endif /* !CONFIG_NVT_JFFS2_SUPPORT && !CONFIG_NVT_SQUASH_SUPPORT */
...
static int nvt_update_rootfsl_partition(u64 addr, u64 size, u64 part_off, u64 part_size, EMB_PARTITION *pEmb)
{
	char command[128];
...
static int nvt_read_fs_partition(u64 addr, u64 size, u64 part_off, u64 part_size, unsigned int pat_id, EMB_PARTITION *pEmb)
{
	char command[128];
	int ret = 0;
	...
	size -= 64;
		...
		unsigned int first_part_size = ALIGN_FLOOR(size, nand_get_block_size());
		unsigned int second_part_size = size - first_part_size;
		sprintf(command, "nand read 0x%llx 0x%llx 0x%llx", addr, part_off, ALIGN_FLOOR(size, nand_get_block_size()));
		run_command(command, 0);
		if(second_part_size > 0)
		{
			sprintf(command, "nand read 0x%llx 0x%llx 0x%llx", (unsigned long)buf, part_off+first_part_size+bad_count*nand_get_block_size(), nand_get_block_size());
			run_command(command, 0);
			memcpy((void*)((unsigned int)addr+first_part_size), (void*)buf, second_part_size);
		}
		free(buf);
	...
	run_command(command, 0);
#elif defined(CONFIG_NVT_LINUX_EMMC_BOOT)
	printf("Read: %s %s size=0x%llx\n", __func__, "EXT4 or FAT", size);
	nvt_sparse_image_readback(addr, part_off, size, part_size);
#endif /* CONFIG_NVT_LINUX_SPINAND_BOOT */


#if defined(CONFIG_NVT_UBIFS_SUPPORT) || (defined(CONFIG_NVT_LINUX_RAMDISK_BOOT) && defined(CONFIG_NVT_SPI_NAND))
	/*test ubifs*/
	printf("Read: %s %s\n", __func__, "UBIFS");
	if(pEmb->EmbType == EMBTYPE_ROOTFS){

		#if (defined(CONFIG_NVT_LINUX_RAMDISK_BOOT) && defined(CONFIG_NVT_SPI_NAND))
			if (pEmb->OrderIdx == 0){
...
static int nvt_read_rootfsl_partition(u64 addr, u64 size, u64 part_off, u64 part_size, unsigned int pat_id, EMB_PARTITION *pEmb)
{
	char command[128];
	int ret = 0;
	...
	sprintf(command, "nand read 0x%llx 0x%llx 0x%llx", addr, part_off, ALIGN_FLOOR(size, nand_get_block_size()));
	run_command(command, 0);
	sprintf(command, "nand read 0x%llx 0x%llx 0x%llx", (unsigned long)buf, part_off+first_part_size, nand_get_block_size());
	run_command(command, 0);
	memcpy((void*)((unsigned int)addr+first_part_size), (void*)buf, second_part_size);
	free(buf);
#elif defined(CONFIG_NVT_SPI_NOR)
...
static int nvt_read_user_partition(u64 addr, u64 size, u64 part_off, u64 part_size, unsigned int pat_id, EMB_PARTITION *pEmb)
{
	char command[128];
	int ret = 0;
...
static int nvt_chk_uboot(ulong addr, unsigned int size, unsigned int pat_id)
{
	...
	if(is_data_area_encrypted())
	{
		//encrypted data , only need check checksum
		return 0;
	}
...
static int nvt_chk_linux(ulong addr, unsigned int size, unsigned int pat_id)
{
	...
	//is_secure = 1;
		...
		is_secure = nvt_ivot_optee_efuse_secure_is_enable();
	#endif
...
{
	int ret = 0;
	#if defined(CONFIG_NVT_LINUX_SPINAND_BOOT) || defined(CONFIG_NVT_LINUX_SPINOR_BOOT) || defined(CONFIG_NVT_LINUX_EMMC_BOOT)|| defined(CONFIG_NVT_LINUX_RAMDISK_BOOT)
	...
	#endif /* CONFIG_NVT_LINUX_SPINAND_BOOT || CONFIG_NVT_LINUX_SPINOR_BOOT || CONFIG_NVT_LINUX_EMMC_BOOT || CONFIG_NVT_LINUX_RAMDISK_BOOT*/
	return ret;
}
...
static int nvt_on_partition_enum_mtd_readback(unsigned int id, NVTPACK_MEM* p_mem, void* p_user_data)
{
	...
	printf("Read back check callback\n");
	switch(pEmb[id].EmbType)
	{
		case EMBTYPE_MBR:
			printf("%s Read: MBR %s\r\n", ANSI_COLOR_YELLOW, ANSI_COLOR_RESET);
