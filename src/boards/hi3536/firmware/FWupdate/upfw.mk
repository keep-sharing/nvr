...
UPFW_CMD_UC   := $(FW_TOPDIR)/update_uc
...
ifeq ($(PLATFORM_TYPE),hi3536)
	$(MS_MAKEFILE_V)cp -dpRf $(UPFW_CMD_EXE) $(UPFW_CMD_UC)
endif
	$(MS_MAKEFILE_V)cp -dpRf $(UPFW_CMD_EXE) $(PUBLIC_APP_DIR)/
	$(MS_MAKEFILE_V)dd if=$(UPFW_CMD_UC) of=./fb1 bs=102400 conv=sync
ifeq ($(PLATFORM_TYPE),hi3536)
	$(MS_MAKEFILE_V)echo -e "ULIBC\c" >> ./fb1
else
	$(MS_MAKEFILE_V)echo -e "GLIBC\c" >> ./fb1
endif
	$(MS_MAKEFILE_V)cp -dpRf $(UPFW_CMD_EXE) ./fb2
	$(MS_MAKEFILE_V)cat ./fb1 ./fb2 > $(UPFW_CMD_EXE)
	$(MS_MAKEFILE_V)rm -f ./fb1 ./fb2 
