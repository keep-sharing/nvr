...
#define DEF_1G_CMDLINE		"mem=1024M console=ttyAMA0,115200 ubi.mtd=fs1 root=ubi0:rootfs rw rootfstype=ubifs"
#define DEF_2G_CMDLINE		"mem=2048M console=ttyAMA0,115200 ubi.mtd=fs1 root=ubi0:rootfs rw rootfstype=ubifs vmalloc=1024M"
...
static inline uint32_t crc_32(const void *buf, unsigned int size)
{
	const uint8_t *p;
	uint32_t crc;
...
int  set_env_boot(int bootid)
{
	...
	if( cmdline == NULL )
    {
        if (check_is_8064())
        {
            memcpy(mtd_table.bootargs, DEF_2G_CMDLINE, sizeof(DEF_2G_CMDLINE));        
        }
        else
        {
            memcpy(mtd_table.bootargs, DEF_1G_CMDLINE, sizeof(DEF_1G_CMDLINE));        
        }
        cmdline = strstr(mtd_table.bootargs, "=fs");
        if( cmdline == NULL )
        {
...
//	oob.ptr = noecc ? G.oobreadbuf : G.oobbuf;
	...
	sprintf(cmd, "/usr/sbin/flash_erase %s 0 0", part_info->mtd_dev);
	if(system(cmd))
	{
