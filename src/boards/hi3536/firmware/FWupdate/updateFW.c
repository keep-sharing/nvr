...
static int check_is_pse_808AR(void)
{
    int res;
    unsigned char *buf;
    int fd;
    ...
    if (res < 0) {
        printf("pse is not ir808ar!!!\n");
        return 0;
    }

    printf("pse is ir808ar!!!\n");
    return 1;
}

static int check_is_7000_ir808ar(void)
{
    FILE *pfd = NULL;
    char buff[64] = {0};
    int result = 0;
    ...
    pfd = popen("mschip_update", "r");
    if (pfd != NULL) {
        if (fgets(buff, sizeof(buff), pfd) != NULL) {//7000系列pse是808AR，与804AR地址相同
            if ((strstr(buff, "MSN7016") || strstr(buff, "MSN7032") || strstr(buff, "MSN7048")) && check_is_pse_808AR()) {
                result = 1;
            }
        }
        pclose(pfd);
...
/*static int mdio_write(int phyaddr, int phyreg, unsigned short phydata)
{
...
int UPFW_verify(char* datafile, mtd_table_t *mtdtable)
{
	...
	if (mtdtable)
	{
		...
		//curoem_No = (mtdtable->part[cur_fsPart].version >> 8) & 0xff;
		if(fwNo <= 7 && curNo <= 7 && check_is_oem()){
			...
			/*if(fwoem_No != curoem_No && fwoem_No != 0) 	
			{
				debug("OEM TYPE do not support update TYPE ERR fwoem_No:%d curoem_No:%d\n", fwoem_No, curoem_No);
				return UPFW_FAILD;
			}*/		
		}
		...
		{
			debug("do not support update < 8.x.10 data 0x%x\n", data);
			return UPFW_FAILD;//not update
		}
#endif
        ...
        //zbing 2017.7.3 add
        if (check_is_8064() && (fwNo == 7) && (curNo == 7) && (fw_subNo < 9) && (cur_subNo >= 9))
        {
...
static int check_is_80xx_new_mcu(void)
{
	char *envinfo;
	char buff[64];
	int plat, sub, ver, result;
	
	result = 0;
	if(!check_is_80xx())
		return 0;
	envinfo = ms_getenv("hard_version"); 
	if(envinfo != NULL)
	{
		strncpy(buff, envinfo, sizeof(buff));
		envinfo = strchr(buff, 'M');
		*envinfo = '\0';
		sscanf(buff, "N%d-%d-V%d-", &plat, &sub, &ver);
		if(sub == 0 && ver < 20)
			result = 0;
		else
			result = 1;
...
int UPFW_version_check(char* datafile)
{
    ...
    if (fwNo <= 7 && curNo <= 7 && check_is_oem()) {
        ...
        debug("8064 do not support update 7.x.9+ to 7.x.8- \n");
        return UPFW_FAILD;
    } else if (check_is_80xx_new_mcu() && fwNo == 9 && fw_subNo < 10) {
        debug("8064 use USART control power do not support update to 9.x.10- \n");
        return UPFW_FAILD;
    } else if (check_is_7000_ir808ar() && fwNo == 9 && fw_subNo < 15) {
        debug("7000_ir808ar do not support update 9+.x.15+ to 9.x.15- \n");
        return UPFW_FAILD;
...
//        debug("debug  current is fs1 \n");
	...
	env_add_msauthcheck(); //goli add to add msauthcheck, when old_version is 63 
