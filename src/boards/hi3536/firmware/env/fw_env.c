...
struct env_image_redundant {
	uint32_t	crc;	/* CRC32 over data bytes    */
//	unsigned char	flags;	/* active or obsolete */ //zbing 2018.10.30 in order to be compatible with older devices
	char		data[];
};
...
//	if (HaveRedundEnv)
//		rc -= sizeof (char);
//zbing 2018.10.30 in order to be compatible with older devices
	return rc;
}
...
int hs_printenv(void)
{
	...
	for (env = environment.data; *env; env = nxt + 1) {
		...
		if(strstr(env, MSKEY) || strstr(env, MSRAN) || strstr(env, MSAUTHCHECK))
			continue;
		else
			printf ("%s\n", env);
	}
...
int fw_printenv (int argc, char *argv[])
{
	...
	for (i = 1; i < argc; ++i) {	/* print single env variables   */
		...
		if(strstr(name, MSKEY) || strstr(name, MSRAN) || strstr(name, MSAUTHCHECK))
			continue;
...
int fw_setenv (int argc, char *argv[])
{
	...
	if (oldval) {
		 ...
		 *///(strcmp (name, "ethaddr") == 0) || // enable update mac .solin
...
static int flash_io (int mode)
{
	...
	if (mode == O_RDWR) {
		...
		rc = flash_write (fd_current, fd_current, dev_current); //bak
...
//      environment.flags   = &redundant->flags;//zbing 2018.10.30 in order to be compatible with older devices
        environment.flags   = (unsigned char *)&redundant->data[ENV_SIZE - 1];
...
//			environment.flag_scheme = FLAG_BOOLEAN;//zbing 2018.10.30 in order to be compatible with older devices
...
//		flag1 = redundant->flags;//zbing 2018.10.30 in order to be compatible with older devices
        flag1 = redundant->data[ENV_SIZE - 1];
		...
		if (dev_current) {
			environment.image	= addr1;
			environment.crc		= &redundant->crc;
//      environment.flags   = &redundant->flags;//zbing 2018.10.30 in order to be compatible with older NVR devices
            environment.flags   = (unsigned char *)&redundant->data[ENV_SIZE - 1];
			environment.data	= redundant->data;
			free (addr0);
		} else {
