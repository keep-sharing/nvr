...
#define HARDWARE_VERSION 	"hard_version"
...
static void set_gpio_defaut(void)
{
	...
	gpio_write(RELAY_OUTPUT0, 0);
	gpio_write(RELAY_OUTPUT1, 0);
	gpio_write(RELAY_OUTPUT2, 0);
	gpio_write(RELAY_OUTPUT3, 0);

    udelay(1000);
	gpio_write(POWEROFF_CTL, 0);
	udelay(10000);
	gpio_write(POWEROFF_CTL, 1);
	udelay(10000);
	gpio_write(POWEROFF_CTL, 0);
	gpio_read(POWEROFF_CTL); //poweroff -> input

    gpio_write(STAT_RED_LED, 0);  // RED LED on
    ...
    //disable FZ&ZM
    //gpio_write(FS_EN_IO, 0);
    //gpio_write(ZM_EN_IO, 0);
    //gpio_write(FZ_AP_IO, 1);
    //gpio_write(FZ_AN_IO, 1);
    //gpio_write(FZ_BN_IO, 1);
    //gpio_write(FZ_BP_IO, 1);
    //disable piris  
    //gpio_write(PIRIS_AP, 1);
    //gpio_write(PIRIS_AN, 1);
    //gpio_write(PIRIS_BP, 1);
    //gpio_write(PIRIS_BN, 1);
    //disable irled
    //gpio_write(IRLED_IO, 0);
}
...
static void show_logo(void)
{
    ...
    if(end != NULL)
    {   
        //clear logo command in old version @bootcmd
        memset(cmd, ' ', end-cmd);
//        printf("bootcmd= %s\n", getenv("bootcmd"));
    }    
   ...
   // run_command("nand read 0x43000000 0 0x80000", 0);
   // run_command("decjpg", 0);
   // run_command("cfgvobg 0 0", 0);
   // run_command("startvo 0 36 12", 0);
   // run_command("startvl 0 0x43800000 1920 0 0 1920 1080", 0);
}
...
static void check_mem_size(void)
{
	...
	if(sBoot)
	{
		memcpy(bootenv, sBoot, strlen(sBoot));

		sMem = strstr(bootenv, "mem=1024"); //mem=1024M -> 1224M 
		if(sMem != NULL)
		{
			sMem[4] = '1';
			sMem[5] = '2';
			...
			printf("Set mem 1024M -> 1224M .... \n");
			setenv("bootargs", bootenv);
		}
		sMem = strstr(bootenv, "mem=2048"); //mem=2048M -> 2148M 
		if(sMem != NULL)
		{
			sMem[4] = '2';
			sMem[5] = '1';
		...
		if(sBoot == NULL)
		{
			printf("===WARNING, bootargs is NULL. \n");
			return;
		}

		memset(bootenv, 0, sizeof(bootenv));
		memcpy(bootenv, sBoot, strlen(sBoot));
		if(strstr(bootenv, "mem=1224") == NULL)
		{
			printf("=bootargs mem != 1224M,[%s] =.\n", bootenv);
		}
#endif
	}
	else
	{
		printf("===WARNING, can't get bootargs env !!! \n");
	}
}
...
static int test_fio(void)
{
	int key1, key2;
	
	key1 = gpio_read(MS_GPIO(2, 4));
	gpio_write(MS_GPIO(2, 5), key1);
	mdelay(100);
	key2 = gpio_read(MS_GPIO(2, 4));
	if(key1 != key2)
		return 1;
	else	
		return 0;
...
static void set_hard_version(char *str)
{
	setenv(HARDWARE_VERSION, str);
	saveenv();
	printf("hard version is %s\n", getenv(HARDWARE_VERSION));
}
 ...
 * 
 *  @ Nxx-aa-Vxx: 
 *  Nxx: 04:8000  03:7000  02:5000  01:1000 series
 *  aa : Product min version num
 *  Vxx: PCB version 10: 1.0
 *
 *  @ Mxx-aa
 *  Mxx: mem size = ddr_version
 *  aa : bit width
 */
static int hard_version_init(void)
{
	char buff[64] = "N04-00-V00-M00-00";
	char *index = NULL;

	index = getenv("chipinfo"); 
	if(index != NULL)
	{
		if(strstr(index, "MSN7") != NULL)
			buff[2] = '3';
	}
	if(test_fio())
		buff[8] = '2';
	buff[13] = (MS_DDR_VERSION == 2) ? '2' : '3';
	
	index = getenv(HARDWARE_VERSION);
	if((NULL == index) || strcmp(index, buff))
		set_hard_version(buff);
