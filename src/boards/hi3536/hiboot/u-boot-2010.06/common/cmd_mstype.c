...
int check(void)
{
	...
	char raddr[CFG_I2C_DM2016_MAX_SIZE] = {0};
#endif

	if (chipinfo_temp == NULL)
	{
		printf("There is no chipinfo stored in the env.\n");
		
#ifdef CONFIG_CMD_DM2016
		int i;
		if (dm2016_read (dev_addr, 0, (uchar *) raddr, DM2016_VAL_LEN) == 0)
		{
			printf("Read value from dm2016 %s \n", raddr);
			
			for(i = 0; i < DM2016_VAL_LEN; i++)
			{
				if (*(raddr + i) == 0)
				{
					printf("dm2016 data is corrupted.\n");
					return -2;
				}
			}
			...
			if (mstype_write(raddr, 0, CHIPINFO_LEN))
			{
				return -1;
			}
		}
		...
		{
			printf("Can not read from dm2016.\n");
			return -3;
		}
#endif
		return -3;
	}
	strncpy(chipinfo, getenv(CHIPINFO), CHIPINFO_LEN);
...
int do_mstype( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	{	
		if (strcmp (argv[1], "write") == 0)
		{
			...
			if (ccode == -1)
				return -1;
			if (ccode == -2 && off != 0 && cnt != CHIPINFO_LEN)
			{
