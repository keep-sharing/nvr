...
static int stmmac_net_set_mac_address(struct stmmac_netdev_local *ld)
{
	...
	if (!is_valid_ether_addr((u8 *)mac)) 
	{
        printf(" Eth addr 2 is not valid! \n");
		set_random_mac_address(mac, ethaddr);
	}
    
    stmmac_hw_set_macaddress2(ld, mac);
...
/*	if (miiphy_read (phy_name, addr, PHY_BMSR, &reg)) {
...
//only for AR8035 delay. hjh.milesight 20151217 add for AR8035 
//-->
//mw 0x11020014 5;mw 0x11020010 0x2743;mw 0x11020014 0x100;mw 0x11020010 0x2783
//mw 0x11020014 0;mw 0x11020010 0x2743;mw 0x11020014 0x8000;mw 0x11020010 0x2783

static int miiphy_delay(struct stmmac_netdev_local *ld)
{
    unsigned char phyaddr;
	char *phy_name;
	...
	unsigned int val;
	unsigned int data = 0;
	reg = 0x1D;  //debug port addr reg
    val = 0x5;  //rgmii tx clock delay
    // add for rtl8211 by shea
    miiphy_read(phy_name, phyaddr, 0x02, &data);
	//printf("data = %0x\n",data);
	if(data==0x4d)
	{
		if (miiphy_write (phy_name, phyaddr, reg, val) != 0) 
		{
			debug ("dev: %s, addr: 0x%x, set phy reg: 0x%x \n", phy_name, phyaddr, reg);
			return (-1);
		}

    	reg = 0x1E; //debug port data reg
    	val = 0x100; //rgmii tx clock delay enable
    ...
    //printf("dev: %s, addr: 0x%x, set phy reg: 0x%x \n", phy_name, phyaddr, reg);
	}
...
#ifdef CONFIG_HI3536
        //printf("========== stmmac_external_phy_reset(GMAC0_PORT); \n");
		stmmac_external_phy_reset(GMAC0_PORT);
#ifndef STMMAC_SINGLE_MAC
        //printf("========== stmmac_external_phy_reset(GMAC1_PORT); \n");
...
retry:
	if (!get_phy_device(GMAC0_PHY_NAME, GMAC0_PHY_ADDR)) {
        //printf("============ get_phy_device,GMAC0_PHY_NAME:[%s] \n", GMAC0_PHY_NAME);
...
#ifndef STMMAC_SINGLE_MAC
	if (!get_phy_device(GMAC1_PHY_NAME, GMAC1_PHY_ADDR)) {
        //printf("============ get_phy_device,GMAC1_PHY_NAME:[%s] \n", GMAC1_PHY_NAME);
...
link_on:
    ...
    //only for AR8035 delay.
    miiphy_delay(&g_stmmac_ld[GMAC0_PORT]);
    //miiphy_delay(&g_stmmac_ld[GMAC1_PORT]);
