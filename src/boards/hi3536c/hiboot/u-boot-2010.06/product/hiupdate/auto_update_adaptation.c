		...
		void *buf)
{
	int ret = 0;
	char *pbuf = buf;
	unsigned long offset_notify, write_start, write_len;
	size_t length;
	...
	while (length > 0) {
		...
		if (length < (nand_flash->erasesize - block_offset))
			rw_size = length;
		else
			rw_size = nand_flash->erasesize - block_offset;
