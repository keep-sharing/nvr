...
# if defined(CONFIG_PHYADDR_TAG)
static void	setup_eth_phyaddr_tag(bd_t * bd,char * phy_addr);
static void setup_eth_physpeed_tag(bd_t *bd, char *phy_speed);
#endif
...
#endif
#ifdef CONFIG_CMDLINE_TAG
    // hugo.milesight add for mtd parts
    char boot_args[1024]={0};
    char *p = strstr(getenv("mtdparts"), "hinand");
    sprintf(boot_args, "%s mtdparts=%s;%s", commandline,  getenv("nor_mtdparts"), p);
	setup_commandline_tag (bd, boot_args);
...
#endif
#if defined(CONFIG_PHYADDR_TAG)
	setup_eth_phyaddr_tag(bd, getenv("phy_addr"));
    setup_eth_physpeed_tag(bd, getenv("phy_speed"));
#endif
#if defined(CONFIG_ETHADDR_TAG)
	setup_ethaddr_tag(bd, getenv("ethaddr"));
    setup_ethaddr_tag(bd, getenv("ethaddr2"));
#endif
#if defined(CONFIG_MULT_CPU_TAG)
...
//zbing add
    ...
    set_param_data("poeNum", (char *)&poeNum, sizeof(int));
    setup_param_tag(bd);
 //zbing end
	setup_end_tag (bd);
#endif
...
static void setup_eth_mdiointf_tag(bd_t *bd, char *mdio_intf)
{
	...
	params->hdr.size = (sizeof (struct tag_header) + strlen (mdio_intf) + 1 + 4) >> 2;;
...
static void setup_eth_phyaddr_tag(bd_t *bd, char *phy_addr)
{
	...
	params->hdr.tag = CONFIG_ETH_PHY_ADDR_TAG_VAL;
	params->hdr.size = 4;
    do{
    	while (*phy_addr == ' ' || *phy_addr == ',')
    		phy_addr++;
        tmp = simple_strtoul(phy_addr, &e, 10);
        if (tmp) {
            *((int *)(&params->u) + index) = tmp;
            phy_addr = e;
            phy_addr = strchr(phy_addr, ',');
        }
    }while(++index < CONFIG_GMAC_NUMS);
...
static void setup_eth_physpeed_tag(bd_t *bd, char *phy_speed)
{
	...
	params->hdr.tag = CONFIG_ETH_PHY_SPEED_TAG_VAL;
	params->hdr.size = 4;
    do{
    	while (*phy_speed == ' ' || *phy_speed == ',')
    		phy_speed++;
        tmp = simple_strtoul(phy_speed, &e, 10);
        *((int *)(&params->u) + index) = tmp;
        phy_speed = e;
        phy_speed = strchr(phy_speed, ',');
    }while(++index < CONFIG_GMAC_NUMS);
