...
static void special_for_8064(char *bootargs)
{
    char *cmdline = NULL;

    cmdline = strstr(bootargs, "mem=1024M");
    if (cmdline) {
        *(cmdline + 4) = '2';
        *(cmdline + 5) = '0';
        *(cmdline + 6) = '4';
    ...
    if (!strstr(bootargs, "vmalloc")) {
        strcat(bootargs, " vmalloc=1024M");
    }
}

static int check_is_x032()
{
    FILE *pfd = NULL;
    char buff[64];
    int result = 0;
...
static void special_for_x032(char *bootargs)
{
    char *cmdline = NULL;
...
static int check_is_pse_804AR(void)
{
    int res;
    unsigned char *buf;
    int fd;
    ...
    ioctl(fd, I2C_SLAVE_FORCE, 0xe0); //设备地址
    ...
    if (res < 0) {
        printf("pse is ir804a!!!\n");
        return 0;
    }

    printf("pse is ir804ar!!!\n");
    return 1;
}

static int check_is_1004_ir804ar(void)
{
    FILE *pfd = NULL;
    char buff[64] = {0};
    int result = 0;
    ...
    if (pfd != NULL) {
        if (fgets(buff, sizeof(buff), pfd) != NULL) {
            if (strstr(buff, "MSN1004") && check_is_pse_804AR()) {
                result = 1;
            }
        }
        pclose(pfd);
...
static int check_is_falsh_MX35LF2G24AD(void)
{
    char *envinfo = NULL;
    int result = 0;

    envinfo = hs_getenv("flashtype");
    if (envinfo && !strcmp(envinfo, "MX35LF2G24AD")) {
        result = 1;
    }
...
static int check_is_1008_unc_or_unpc(void)
{
    int result = 0;
    char product[64] = {0};
    char *chipinfo = NULL;

    chipinfo = hs_getenv("chipinfo");
    if (chipinfo) {
        strncpy(product, chipinfo, sizeof(product));
        if (strstr(product, "MSN1008") && product[14] == 'N') {
            result = 1;
        }
    }
...
int UPFW_version_check(char *datafile)
{
    ...
    if (fwNo <= 7 && curNo <= 7 && check_is_oem()) {
        ...
        debug("do not support back to < 9.0.x from 9.0.x. \n");
        return UPFW_FAILD; //not update
    } else if (check_is_8064() && (fwNo == 7) && (curNo == 7) && (fw_subNo < 9) && (cur_subNo >= 9)) {
        debug("8064 do not support update 7.x.9+ to 7.x.8- \n");
        return UPFW_FAILD;
    } else if (check_is_5008() && fwNo == 9 && fw_subNo < 10) {
        debug("5008 do not support update 9+.x.10+ to 9-.x.10- \n");
        return UPFW_FAILD;
    } else if (check_is_1008_unc_or_unpc() && fwNo == 9 && fw_subNo < 15) {     //goli add for 1008-unc/unpc 2022.04.24
        debug("1008UNC/UNPC do not support update 9+.x.15+ to 9.x.15- \n");
        return UPFW_FAILD;
    } else if (check_is_1008() && fwNo == 9 && fw_subNo < 12) {
        /* Promblem Number: ID1045114     Author:wuzh,   Date:2021/4/15
           Description    : 1008-UXC机型(9.0.11-r2才兼容),不允许降级至9.0.11及以下版本 */
        debug("1008 do not support update 9+.x.12+ to 9.x.12- \n");
        return UPFW_FAILD;
    } else if (check_is_1004_ir804ar() && fwNo == 9 && fw_subNo < 14) {
        debug("1004_ir804ar do not support update 9+.x.14+ to 9.x.14- \n");
        return UPFW_FAILD;
    } else if (check_is_falsh_MX35LF2G24AD() && fwNo == 9 && fw_subNo < 15) {   //goli add for new_flash 2022.04.22
        debug("new flash[MX35LF2G24AD] do not support update 9+.x.15+ to 9.x.15- \n");
        return UPFW_FAILD;
...
//        debug("debug  current is fs1 \n");
    ...
    if ((header_info.img_head[UPFW_BLD].fw_len > 0)
        ...
        if (check_is_x032()) { //if x032 use 2GB mem && mem=1024 --> mem=1224
            special_for_x032(mtd_table.bootargs);
        }
    }
    ...
    if (check_is_8064()) {
        //force uboot to 4G
        firmware_flag[UPFW_BLD] = 0;
        ptb_info[UPFW_BLD].fw_pad = 0;
        if (((header_info.img_head[UPFW_BLD2].fw_version & 0xff) >
             (mtd_table.part[PART_BLD].version & 0xff))) {
            firmware_flag[UPFW_BLD2] = 1;
            ptb_info[UPFW_BLD2].fw_pad = 1;
            strcpy(ptb_info[UPFW_BLD2].mtd_name, mtd_table.part[PART_BLD].name);
            strcpy(ptb_info[UPFW_BLD2].mtd_dev, mtd.mtd_info[PART_BLD].mtd_dev);
            special_for_8064(mtd_table.bootargs);
        }
    }
    //zbing add 2017.6.30 end
#ifdef INPUT_JPG_IMG
