...
swend:
	//bruce.milesight add version:4.0.71 2014.04.04
	if ( DhcpOptions.val[ntpServers] )
    	{
    		char ntp_server[64] = {0};
		int j = 0;
		//for (j=0;j<DhcpOptions.len[ntpServers];j+=4)//get first one
		{
			snprintf(ntp_server, sizeof(ntp_server), "%u.%u.%u.%u", 		
				((unsigned char *)DhcpOptions.val[ntpServers])[0+j],
				((unsigned char *)DhcpOptions.val[ntpServers])[1+j],
				((unsigned char *)DhcpOptions.val[ntpServers])[2+j],
				((unsigned char *)DhcpOptions.val[ntpServers])[3+j]);
				FILE *fp = fopen("/tmp/dhcp_ntp", "wb");
				if(fp)
				{
					fwrite(ntp_server, 1, strlen(ntp_server), fp);
					fclose(fp);
				}			
		}
	}
	//end bruce.milesight add ntp server	
#ifdef DEBUG
...
UdpIpMsgRecv.ethhdr.ether_shost[5]);
  ...
  if ( ! DhcpOptions.val[dhcpServerIdentifier] ) /* did not get dhcpServerIdentifier */
    {	/* make it the same as IP address of the sender */
      struct udpiphdr *updRecv = (struct udpiphdr *)UdpIpMsgRecv.udpipmsg;
      struct ip *ipRecv=(struct ip *)updRecv->ip;
...
void *dhcpStart()
{
  ...
  if ( setsockopt(udpFooSocket,SOL_SOCKET,SO_BROADCAST,&o,sizeof(o)) )
    syslog(LOG_ERR,"dhcpStart: setsockopt: %m\n");
  memset(&clientAddr.sin_addr,0,sizeof(struct in_addr));
