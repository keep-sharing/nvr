...
/*****************************************************************************/
#define TEMP_RESOLV_CONF RESOLV_CONF
int net_has_dns(char *dns)
{
	FILE *fp;
	char dnsname[80]={0}, buffer[256] = {0};
	int ret = 0;
	fp = fopen(TEMP_RESOLV_CONF, "rb");	
	if(!fp)
		return 0;
...
int net_add_dns(char *dns)
{
	FILE *fp;
	char buffer[256] = {0};
	if(net_has_dns(dns))
		return 0;
	fp = fopen(TEMP_RESOLV_CONF, "ab+");
	if(!fp)
		return -1;
...
int dhcpConfig()
{
  ...
  if ( ReplResolvConf )
    {
      //resolv_renamed=1+rename(RESOLV_CONF,""RESOLV_CONF".sv");//bruce.milesight delete.
	int i = 0;
	char dns_info[128]={0};
	int have_found_dns = 0;
	for (i=0;i<DhcpOptions.len[dns];i+=4)
	{
		snprintf(dns_info, sizeof(dns_info), "%u.%u.%u.%u",   
		...
		net_add_dns(dns_info);
		if(IfName && have_found_dns == 0)
		{
			char sFileName[32] = {0};
			FILE* tmp = NULL;
			snprintf(sFileName, sizeof(sFileName), "/tmp/%s.dns", IfName);
			if((tmp = fopen(sFileName, "w")) != NULL)
			{
				fwrite(dns_info, strlen(dns_info), 1, tmp);
				fclose(tmp);
				tmp = NULL;
				have_found_dns = 1;
			}
...
#endif
//bruce	  for (i=0;i<DhcpOptions.len[dns];i+=4)
//bruce	    fprintf(f,"nameserver %u.%u.%u.%u\n",
//bruce	    ((unsigned char *)DhcpOptions.val[dns])[i],
//bruce	    ((unsigned char *)DhcpOptions.val[dns])[i+1],
//bruce	    ((unsigned char *)DhcpOptions.val[dns])[i+2],
//bruce	    ((unsigned char *)DhcpOptions.val[dns])[i+3]);
#if 0
...
#else
//bruce	  if ( DhcpOptions.len[domainName] )
//bruce	    fprintf(f,"search %s\n",(char *)DhcpOptions.val[domainName]);
#endif
//bruce	  fclose(f);
//bruce	}
//bruce      else
//bruce	syslog(LOG_ERR,"dhcpConfig: fopen: %m\n");
...
tsc:
  ...
  strncpy((char*)DhcpIface.version,VERSION,sizeof(DhcpIface.version));
