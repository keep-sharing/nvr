...
#include "ui_AdvancedMotionDetection.h"
...
const MsQtVca::ObjectVcaType ObjectType = MsQtVca::AdvancedMotionDetection;

AdvancedMotionDetection::AdvancedMotionDetection(QWidget *parent)
    : BaseSmartWidget(parent)
    , ui(new Ui::AdvancedMotionDetection)
{
    ui->setupUi(this);
    ...
    ui->comboBox_IgnoreShortLiveMotion->clear();
    ui->comboBox_IgnoreShortLiveMotion->addItem(GET_TEXT("COMMON/1013", "Off"), 0);
    ui->comboBox_IgnoreShortLiveMotion->addItem("1s", 1);
    ui->comboBox_IgnoreShortLiveMotion->addItem("2s", 2);
    ui->comboBox_IgnoreShortLiveMotion->addItem("3s", 3);
    ui->comboBox_IgnoreShortLiveMotion->addItem("4s", 4);
    ui->comboBox_IgnoreShortLiveMotion->addItem("5s", 5);
    ...
    m_effective = new EffectiveTimeVCA(this);
...
AdvancedMotionDetection::~AdvancedMotionDetection()
{
    if (m_event_info) {
        delete m_event_info;
...
void AdvancedMotionDetection::initializeData(int channel)
{
    //这里会调用onChannelButtonClicked(channel)槽函数
    ui->widget_buttonGroup->setCurrentIndex(channel);
}

void AdvancedMotionDetection::saveData()
{
    if (!checkObjectSize()) {
        clearCopyInfo();
        return;
    ...
    if (cameraVersion >= MsCameraVersion(7, 76)) {
        m_event_info->ignore_time = ui->comboBox_IgnoreShortLiveMotion->currentIndex();
    }

    sendMessage(REQUEST_FLAG_SET_VCA_ADVANCEDMOTION, m_event_info, sizeof(ms_smart_event_info));
    //m_eventLoop.exec();
...
void AdvancedMotionDetection::copyData()
{
    ...
    if (result == ChannelCopyDialog::Accepted) {
        ...
        saveData();
        copy_smart_event_action(ADVANCED_MOTION, currentChannel(), copyFlags, 1);

    }
}

void AdvancedMotionDetection::clearCopyInfo()
{
    for (int i = 0; i < MAX_LEN_65; i++) {
        m_event_info->copyChn[i] = '0';
        m_settings_info2->copyChn[i] = '0';
...
void AdvancedMotionDetection::processMessage(MessageReceive *message)
{
    dealGlobalMessage(message);
    //
    switch (message->type()) {
    case RESPONSE_FLAG_GET_VCA_ADVANCEDMOTION:
        ON_RESPONSE_FLAG_GET_VAC_ADVANCEDMOTION(message);
        break;
    case RESPONSE_FLAG_SET_VCA_ADVANCEDMOTION:
        ON_RESPONSE_FLAG_SET_VCA_ADVANCEDMOTION(message);
        break;
...
void AdvancedMotionDetection::ON_RESPONSE_FLAG_GET_VAC_ADVANCEDMOTION(MessageReceive *message)
{
    if (message->isNull()) {
    ...
    MsCameraVersion cameraVersion = MsCameraVersion::fromChannel(currentChannel());
    if (cameraVersion >= MsCameraVersion(7, 76)  && !qMsNvr->isSigma(currentChannel())) {
        ui->label_IgnoreShortLiveMotion->show();
        ui->comboBox_IgnoreShortLiveMotion->show();
        ui->comboBox_IgnoreShortLiveMotion->setCurrentIndex(m_event_info->ignore_time);
    } else {
        ui->label_IgnoreShortLiveMotion->hide();
        ui->comboBox_IgnoreShortLiveMotion->hide();
    }
    //【QT-Event-VCA：高级移动检测界面，sigma平台IPC要屏蔽物体尺寸大小配置选项】https://www.tapd.cn/21417271/bugtrace/bugs/view?bug_id=1121417271001075155
    //【QT-VCA-Advanced Motion Detection：Sigma平台机型，从77版本开始，支持物体尺寸配置项，需兼容】https://www.tapd.cn/21417271/bugtrace/bugs/view?bug_id=1121417271001093589
    if (qMsNvr->isSigma(currentChannel()) && MsCameraVersion(cameraVersion.majorVersion(), cameraVersion.minorVersion()) <= MsCameraVersion(7, 76)) {
        ui->widgetSize->hide();
        ui->labelObjectSizeLimits->hide();
        ui->widgetObjectSize->hide();
    }
...
void AdvancedMotionDetection::ON_RESPONSE_FLAG_SET_VCA_ADVANCEDMOTION(MessageReceive *message)
{
    if (message->isNull()) {
...
void AdvancedMotionDetection::ON_RESPONSE_FLAG_GET_VAC_SETTINGS2(MessageReceive *message)
{
    if (message->isNull()) {
        m_eventLoop.exit(-1);
        return;
...
void AdvancedMotionDetection::ON_RESPONSE_FLAG_SET_VAC_SETTINGS2(MessageReceive *message)
{
    if (message->isNull()) {
        qMsWarning() << "data is null.";
        m_eventLoop.exit(-1);
...
void AdvancedMotionDetection::clearSettings()
{
...
void AdvancedMotionDetection::updateEnableState()
{
    ...
    ui->comboBox_IgnoreShortLiveMotion->setEnabled(isEnable);
...
bool AdvancedMotionDetection::checkObjectSize()
{
...
bool AdvancedMotionDetection::isSupportPolygon()
{
...
void AdvancedMotionDetection::setEffectiveRegionVisible(bool visible)
{
    if (visible) {
...
void AdvancedMotionDetection::showRegionEdit()
{
    ui->commonVideo->hideDrawView();
    if (isSupportPolygon()) {
        m_drawMultiControl->show();
...
void AdvancedMotionDetection::readRegion(int index)
{
...
void AdvancedMotionDetection::restoreUI()
{
    ui->labelRegionNo->show();
    ui->comboBoxRegionNo->show();
    ...
    ui->widgetSize->show();
    ui->labelObjectSizeLimits->show();
    ui->widgetObjectSize->show();
    ui->label_IgnoreShortLiveMotion->show();
    ui->comboBox_IgnoreShortLiveMotion->show();
...
void AdvancedMotionDetection::onLanguageChanged()
{
    ui->label_advancedMotion->setText(GET_TEXT("SMARTEVENT/55003", "Advanced Motion Detection"));
    ui->label_setRegion->setText(GET_TEXT("ANPR/103021", "Region"));
    ui->label_sensitivity->setText(GET_TEXT("MOTION/51003", "Sensitivity"));
    ui->label_IgnoreShortLiveMotion->setText(GET_TEXT("MOTION/51008", "Ignore Short-Lived Motion"));
...
void AdvancedMotionDetection::onChannelButtonClicked(int index)
{
    if (!isVisible()) {
        return;
    }
    ...
    //获取数据
    // int result = 0;
    // sendMessage(REQUEST_FLAG_GET_VCA_ADVANCEDMOTION, (void *)&channel, sizeof(int));
...
void AdvancedMotionDetection::on_pushButton_setAll_clicked()
{
    if (isSupportPolygon()) {
...
void AdvancedMotionDetection::on_pushButton_clearAll_clicked()
{
    if (isSupportPolygon()) {
...
void AdvancedMotionDetection::on_pushButton_editTime_clicked()
{
    m_effective->showEffectiveTime(currentChannel(), ADVANCED_MOTION);
}

void AdvancedMotionDetection::on_pushButton_editAction_clicked()
{
    m_action->showAction(currentChannel(), ADVANCED_MOTION);
}

void AdvancedMotionDetection::on_checkBoxEnable_clicked(bool checked)
{
    Q_UNUSED(checked)
    if (m_event_info->regionType == REGION_SINGLE) {
...
void AdvancedMotionDetection::onSceneObjectSizeChanged(const QRect &rc, MsQtVca::ObjectSizeType sizeType)
{
    switch (sizeType) {
...
void AdvancedMotionDetection::on_radioButton_minLayout_clicked(bool checked)
{
    if (!checked) {
...
void AdvancedMotionDetection::on_radioButton_maxLayout_clicked(bool checked)
{
    if (!checked) {
...
void AdvancedMotionDetection::on_pushButtonRegionEdit_clicked()
{
...
void AdvancedMotionDetection::on_pushButtonRegionEdit_stateSet(PushButtonEditState::State state)
{
...
void AdvancedMotionDetection::on_pushButtonObjectSizeEdit_clicked()
{
...
void AdvancedMotionDetection::on_pushButtonObjectSizeEdit_stateSet(PushButtonEditState::State state)
{
    if (ui->checkBoxEnable->isEnabled()) {
        switch (state) {
        case PushButtonEditState::StateFinished:
...
void AdvancedMotionDetection::on_pushButtonObjectSizeReset_clicked()
{
...
void AdvancedMotionDetection::onLineEditMinWidthEditingFinished()
{
...
void AdvancedMotionDetection::onLineEditMinHeightEditingFinished()
{
...
void AdvancedMotionDetection::onLineEditMaxWidthEditingFinished()
{
...
void AdvancedMotionDetection::onLineEditMaxHeightEditingFinished()
{
...
void AdvancedMotionDetection::onDrawPolygonConflicted()
{
    MessageBox::information(this, GET_TEXT("SMARTEVENT/55066", "The boundaries of the area cannot intersect. Please redraw."));
    m_drawMultiControl->clearPolygon();
}

void AdvancedMotionDetection::on_comboBoxEffectiveRegion_indexSet(int index)
{
...
void AdvancedMotionDetection::on_comboBoxEffectiveWithPreset_activated(int index)
{
...
void AdvancedMotionDetection::on_comboBoxRegionNo_indexSet(int index)
{
    ui->pushButtonRegionEdit->setButtonState(PushButtonEditState::StateFinished);
    ui->pushButtonObjectSizeEdit->setButtonState(PushButtonEditState::StateFinished);
...
void AdvancedMotionDetection::on_pushButtonEffectiveWithPreset_clicked()
{
    int index = ui->comboBoxEffectiveWithPreset->currentIndex();
    gPtzDataManager->sendPresetControl(PTZ_PRESET_GOTO, index + 1, 5, ui->comboBoxEffectiveWithPreset->currentData().toString());
}

void AdvancedMotionDetection::on_checkBoxHuman_clicked()
{
    if (m_event_info->regionType == REGION_SINGLE) {
...
void AdvancedMotionDetection::on_checkBoxVehicle_clicked()
{
    if (m_event_info->regionType == REGION_SINGLE) {
