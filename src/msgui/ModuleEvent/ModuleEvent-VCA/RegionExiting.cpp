...
const MsQtVca::ObjectVcaType ObjectType = MsQtVca::RegionExiting;

RegionExiting::RegionExiting(QWidget *parent)
    : BaseSmartWidget(parent)
    , ui(new Ui::RegionExiting)
{
    ui->setupUi(this);
...
void RegionExiting::saveData()
{
    ...
    sendMessage(REQUEST_FLAG_SET_VCA_REGIONEXIT, m_event_info, sizeof(ms_smart_event_info));
    m_eventLoop.exec();
...
void RegionExiting::copyData()
{
    ...
    if (result == ChannelCopyDialog::Accepted) {
        ...
        saveData();
        copy_smart_event_action(REGIONOUT, currentChannel(), copyFlags, 1);
    }
}

void RegionExiting::clearCopyInfo()
{
    for (int i = 0; i < MAX_LEN_65; i++) {
        m_event_info->copyChn[i] = '0';
        m_settings_info2->copyChn[i] = '0';
...
void RegionExiting::processMessage(MessageReceive *message)
{
    ...
    case RESPONSE_FLAG_GET_VCA_REGIONEXIT:
        ...
        break;
    case RESPONSE_FLAG_SET_VCA_REGIONEXIT:
        ON_RESPONSE_FLAG_SET_VCA_REGIONEXIT(message);
        break;
...
void RegionExiting::ON_RESPONSE_FLAG_GET_VCA_REGIONEXIT(MessageReceive *message)
{
    if (message->isNull()) {
...
void RegionExiting::ON_RESPONSE_FLAG_SET_VCA_REGIONEXIT(MessageReceive *message)
{
    if (message->isNull()) {
...
void RegionExiting::ON_RESPONSE_FLAG_GET_VAC_SETTINGS2(MessageReceive *message)
{
    if (message->isNull()) {
        m_eventLoop.exit(-1);
        return;
...
void RegionExiting::ON_RESPONSE_FLAG_SET_VAC_SETTINGS2(MessageReceive *message)
{
    if (message->isNull()) {
        qMsWarning() << "data is null.";
        m_eventLoop.exit(-1);
...
void RegionExiting::clearSettings()
{
...
void RegionExiting::updateEnableState()
{
...
bool RegionExiting::checkObjectSize()
{
...
bool RegionExiting::isSupportPolygon()
{
...
void RegionExiting::setEffectiveRegionVisible(bool visible)
{
    if (visible) {
...
void RegionExiting::showRegionEdit()
{
    ui->commonVideo->hideDrawView();
    if (isSupportPolygon()) {
        m_drawMultiControl->show();
...
void RegionExiting::readRegion(int index)
{
...
void RegionExiting::onLanguageChanged()
{
    ui->label_region->setText(GET_TEXT("SMARTEVENT/55002", "Region Exiting"));
...
void RegionExiting::onChannelButtonClicked(int index)
{
    if (!isVisible()) {
        return;
    }
    ...
    // int result = 0;
    // sendMessage(REQUEST_FLAG_GET_VCA_REGIONEXIT, (void *)&channel, sizeof(int));
...
void RegionExiting::on_checkBoxEnable_clicked(bool checked)
{
    Q_UNUSED(checked)
    if (m_event_info->regionType == REGION_SINGLE) {
...
void RegionExiting::on_pushButton_setAll_clicked()
{
    if (isSupportPolygon()) {
...
void RegionExiting::on_pushButton_clearAll_clicked()
{
    if (isSupportPolygon()) {
...
void RegionExiting::onSceneObjectSizeChanged(const QRect &rc, MsQtVca::ObjectSizeType sizeType)
{
    switch (sizeType) {
...
void RegionExiting::on_radioButton_minLayout_clicked(bool checked)
{
    if (!checked) {
...
void RegionExiting::on_radioButton_maxLayout_clicked(bool checked)
{
    if (!checked) {
...
void RegionExiting::onLineEditMinWidthEditingFinished()
{
...
void RegionExiting::onLineEditMinHeightEditingFinished()
{
...
void RegionExiting::onLineEditMaxWidthEditingFinished()
{
...
void RegionExiting::onLineEditMaxHeightEditingFinished()
{
...
void RegionExiting::on_pushButtonRegionEdit_clicked()
{
...
void RegionExiting::on_pushButtonRegionEdit_stateSet(PushButtonEditState::State state)
{
...
void RegionExiting::on_pushButtonObjectSizeEdit_clicked()
{
...
void RegionExiting::on_pushButtonObjectSizeEdit_stateSet(PushButtonEditState::State state)
{
    if (ui->checkBoxEnable->isEnabled()) {
        switch (state) {
        case PushButtonEditState::StateFinished:
...
void RegionExiting::on_pushButtonObjectSizeReset_clicked()
{
...
void RegionExiting::on_pushButton_editTime_clicked()
{
    if (!m_effectiveTime) {

        m_effectiveTime = new EffectiveTimeVCA(this);
    }
    m_effectiveTime->showEffectiveTime(currentChannel(), REGIONOUT);
}

void RegionExiting::on_pushButton_editAction_clicked()
{
    int channel = currentChannel();
    if (!m_action) {
        m_action = new ActionSmartEvent(this);
    }
    m_action->showAction(channel, REGIONOUT);
}

void RegionExiting::onDrawPolygonConflicted()
{
    MessageBox::information(this, GET_TEXT("SMARTEVENT/55066", "The boundaries of the area cannot intersect. Please redraw."));
    m_drawMultiControl->clearPolygon();
}

bool RegionExiting::needSensitivity()
{
    return m_needSensitivity;
}

void RegionExiting::setNeedSensitivity(bool b)
{
...
bool RegionExiting::isSupportSensitivity(int channel)
{
...
void RegionExiting::onNeedSensitivityChanged()
{
...
void RegionExiting::on_checkBoxHuman_clicked()
{
    if (m_event_info->regionType == REGION_SINGLE) {
...
void RegionExiting::on_checkBoxVehicle_clicked()
{
    if (m_event_info->regionType == REGION_SINGLE) {
...
void RegionExiting::on_comboBoxEffectiveRegion_indexSet(int index)
{
...
void RegionExiting::on_comboBoxEffectiveWithPreset_activated(int index)
{
...
void RegionExiting::on_comboBoxRegionNo_indexSet(int index)
{
    ui->pushButtonRegionEdit->setButtonState(PushButtonEditState::StateFinished);
    ui->pushButtonObjectSizeEdit->setButtonState(PushButtonEditState::StateFinished);
...
void RegionExiting::on_pushButtonEffectiveWithPreset_clicked()
{
