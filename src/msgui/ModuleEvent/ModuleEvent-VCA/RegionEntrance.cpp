...
const MsQtVca::ObjectVcaType ObjectType = MsQtVca::RegionEntrance;

RegionEntrance::RegionEntrance(QWidget *parent)
    : BaseSmartWidget(parent)
    , ui(new Ui::RegionEntrance)
{
    ui->setupUi(this);
...
void RegionEntrance::saveData()
{
    ...
    if (needSensitivity()) {
        m_event_info->sensitivity = ui->slider_sensitivity->value();
    }

    sendMessage(REQUEST_FLAG_SET_VCA_REGIONENTRANCE, m_event_info, sizeof(ms_smart_event_info));
    m_eventLoop.exec();
...
void RegionEntrance::copyData()
{
    ...
    if (result == ChannelCopyDialog::Accepted) {
        ...
        saveData();
        copy_smart_event_action(REGIONIN, currentChannel(), copyFlags, 1);
    }
}

void RegionEntrance::clearCopyInfo()
{
    for (int i = 0; i < MAX_LEN_65; i++) {
        m_event_info->copyChn[i] = '0';
        m_settings_info2->copyChn[i] = '0';
...
void RegionEntrance::processMessage(MessageReceive *message)
{
    ...
    case RESPONSE_FLAG_GET_VCA_REGIONENTRANCE:
        ...
        break;
    case RESPONSE_FLAG_SET_VCA_REGIONENTRANCE:
        ON_RESPONSE_FLAG_SET_VCA_REGIONENTRANCE(message);
        break;
...
void RegionEntrance::ON_RESPONSE_FLAG_GET_VCA_REGIONENTRANCE(MessageReceive *message)
{
    if (message->isNull()) {
    ...
    ui->slider_sensitivity->setValue(m_event_info->sensitivity);

    m_eventLoop.exit();
}

void RegionEntrance::ON_RESPONSE_FLAG_SET_VCA_REGIONENTRANCE(MessageReceive *message)
{
    if (message->isNull()) {
...
void RegionEntrance::ON_RESPONSE_FLAG_GET_VAC_SETTINGS2(MessageReceive *message)
{
    if (message->isNull()) {
        m_eventLoop.exit(-1);
        return;
...
void RegionEntrance::ON_RESPONSE_FLAG_SET_VAC_SETTINGS2(MessageReceive *message)
{
    if (message->isNull()) {
        qMsWarning() << "data is null.";
        m_eventLoop.exit(-1);
...
void RegionEntrance::clearSettings()
{
...
void RegionEntrance::updateEnableState()
{
...
bool RegionEntrance::checkObjectSize()
{
...
bool RegionEntrance::isSupportPolygon()
{
...
void RegionEntrance::setEffectiveRegionVisible(bool visible)
{
    if (visible) {
...
void RegionEntrance::showRegionEdit()
{
    ui->commonVideo->hideDrawView();
    if (isSupportPolygon()) {
        m_drawMultiControl->show();
...
void RegionEntrance::readRegion(int index)
{
...
void RegionEntrance::onLanguageChanged()
{
    ui->label_region->setText(GET_TEXT("SMARTEVENT/55001", "Region Entrance"));
...
void RegionEntrance::onChannelButtonClicked(int index)
{
    if (!isVisible()) {
        return;
    }
    ...
    // int result = 0;
    // sendMessage(REQUEST_FLAG_GET_VCA_REGIONENTRANCE, (void *)&channel, sizeof(int));
...
void RegionEntrance::on_checkBoxEnable_clicked(bool checked)
{
    Q_UNUSED(checked)
    if (m_event_info->regionType == REGION_SINGLE) {
...
void RegionEntrance::on_pushButton_setAll_clicked()
{
    if (isSupportPolygon()) {
...
void RegionEntrance::on_pushButton_clearAll_clicked()
{
    if (isSupportPolygon()) {
...
void RegionEntrance::onSceneObjectSizeChanged(const QRect &rc, MsQtVca::ObjectSizeType sizeType)
{
    switch (sizeType) {
...
void RegionEntrance::on_radioButton_minLayout_clicked(bool checked)
{
    if (!checked) {
...
void RegionEntrance::on_radioButton_maxLayout_clicked(bool checked)
{
    if (!checked) {
...
void RegionEntrance::on_pushButtonRegionEdit_clicked()
{
...
void RegionEntrance::on_pushButtonRegionEdit_stateSet(PushButtonEditState::State state)
{
...
void RegionEntrance::on_pushButtonObjectSizeEdit_clicked()
{
...
void RegionEntrance::on_pushButtonObjectSizeEdit_stateSet(PushButtonEditState::State state)
{
    if (ui->checkBoxEnable->isEnabled()) {
        switch (state) {
        case PushButtonEditState::StateFinished:
...
void RegionEntrance::on_pushButtonObjectSizeReset_clicked()
{
...
void RegionEntrance::on_pushButton_editTime_clicked()
{
    if (!m_effectiveTime) {

        m_effectiveTime = new EffectiveTimeVCA(this);
    }
    m_effectiveTime->showEffectiveTime(currentChannel(), REGIONIN);
}

void RegionEntrance::on_pushButton_editAction_clicked()
{
    int channel = currentChannel();
    if (!m_action) {
        m_action = new ActionSmartEvent(this);
    }
    m_action->showAction(channel, REGIONIN);
}

bool RegionEntrance::needSensitivity()
{
    return m_needSensitivity;
}

void RegionEntrance::setNeedSensitivity(bool b)
{
...
bool RegionEntrance::isSupportSensitivity(int channel)
{
...
void RegionEntrance::onNeedSensitivityChanged()
{
...
void RegionEntrance::onLineEditMinWidthEditingFinished()
{
...
void RegionEntrance::onLineEditMinHeightEditingFinished()
{
...
void RegionEntrance::onLineEditMaxWidthEditingFinished()
{
...
void RegionEntrance::onLineEditMaxHeightEditingFinished()
{
...
void RegionEntrance::on_comboBoxEffectiveRegion_indexSet(int index)
{
    //normal->advancedd需要先判断是否有预置点
...
void RegionEntrance::on_comboBoxEffectiveWithPreset_activated(int index)
{
...
void RegionEntrance::on_checkBoxHuman_clicked()
{
    if (m_event_info->regionType == REGION_SINGLE) {
...
void RegionEntrance::on_checkBoxVehicle_clicked()
{
    if (m_event_info->regionType == REGION_SINGLE) {
...
void RegionEntrance::on_pushButtonEffectiveWithPreset_clicked()
{
