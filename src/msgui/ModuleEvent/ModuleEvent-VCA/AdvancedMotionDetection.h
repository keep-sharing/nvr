#ifndef ADVANCEDMOTIONDETECTION_H
#define ADVANCEDMOTIONDETECTION_H
...
struct smart_event;
namespace Ui {
class AdvancedMotionDetection;
}

class AdvancedMotionDetection : public BaseSmartWidget {
    Q_OBJECT

public:
    explicit AdvancedMotionDetection(QWidget *parent = 0);
...
private:
    void ON_RESPONSE_FLAG_GET_VAC_ADVANCEDMOTION(MessageReceive *message);
    void ON_RESPONSE_FLAG_SET_VCA_ADVANCEDMOTION(MessageReceive *message);
...
protected:
    Ui::AdvancedMotionDetection *ui;
    DrawMotion *m_videoDraw = nullptr;

    ActionSmartEvent *m_action = nullptr;
    EffectiveTimeVCA *m_effective = nullptr;
...
#endif // ADVANCEDMOTIONDETECTION_H
