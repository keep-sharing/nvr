...
//	printf("resolution(%d, %d)\n", frame->width, frame->height);
	...
	if(p_rtsp_info->handle_server && (frame->strm_type == ST_VIDEO || frame->strm_type == ST_AUDIO))
	{		
		if(p_rtsp_info->state_server == 2)
			reco_rtsp_server_senddata(p_rtsp_info->handle_server, frame);
		rtsp_write_data(frame, 1);
//        printf("11resolution(%d, %d)\n", frame->width, frame->height);
...
int ms_rtsp_create_client(int chan_id, int tcp_udp, int rtsp_port, char *ip_addr, char *username, char *password, enum stream_type enType)
{
	...
	param.mainsub   = (enType == STREAM_TYPE_MAINSTREAM);
	param.tindex    = g_rtsp.tindex++;
	param.streamfmt = enType;
	param.fps       = 25;
	param.bitrate   = 4096;
...
{
	struct rtsp_server_args desc = {0};
