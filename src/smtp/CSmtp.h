...
#define TIME_IN_SEC		30		// how long client will wait for server response in non-blocking mode bruce.milesight modity 180 to 30 seconds
#define BUFFER_SIZE 10240	  // SendData and RecvData buffers sizes
#define FILE_READ_BUFF_SIZE 540//bruce.milesight add version:2.0.7 one times read file size
//bruce.milesight modify 10 to 50 for MJPEG is too big.2014.09.12
...
#define SMTP_LOG_PATH "/tmp/smtp.log"//bruce.milesight add
...
public:
	...
	{
		FILE *fp = fopen(SMTP_LOG_PATH, "wb");
		if(fp)
		{
			fprintf(fp, "%d\n%s\n", ErrorCode, GetErrorText().c_str());
			fflush(fp);
			fclose(fp);
		}
		printf("smtp error:%s\n", GetErrorText().c_str());
...
private:
    ...
    bool m_bQuitNoResponse; // wyl add for quit no response
};
