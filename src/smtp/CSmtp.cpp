...
Command_Entry command_list[] = 
{
	{command_INIT,          0,     1*30,  220, ECSmtp::SERVER_NOT_RESPONDING},
	{command_EHLO,          1*30,  1*30,  250, ECSmtp::COMMAND_EHLO},
	{command_AUTHPLAIN,     1*30,  1*30,  235, ECSmtp::COMMAND_AUTH_PLAIN},
	{command_AUTHLOGIN,     1*30,  1*30,  334, ECSmtp::COMMAND_AUTH_LOGIN},
	{command_AUTHCRAMMD5,   1*30,  1*30,  334, ECSmtp::COMMAND_AUTH_CRAMMD5},
	{command_AUTHDIGESTMD5, 1*30,  1*30,  334, ECSmtp::COMMAND_AUTH_DIGESTMD5},
	{command_DIGESTMD5,     1*30,  1*30,  335, ECSmtp::COMMAND_DIGESTMD5},
	{command_USER,          1*30,  1*30,  334, ECSmtp::UNDEF_XYZ_RESPONSE},
	{command_PASSWORD,      1*30,  1*30,  235, ECSmtp::BAD_LOGIN_PASS},
	{command_MAILFROM,      1*30,  1*30,  250, ECSmtp::COMMAND_MAIL_FROM},
	{command_RCPTTO,        1*30,  1*30,  250, ECSmtp::COMMAND_RCPT_TO},
	{command_DATA,          1*30,  2*60,  354, ECSmtp::COMMAND_DATA},
	{command_DATABLOCK,     3*30,  0,     0,   ECSmtp::COMMAND_DATABLOCK},	// Here the valid_reply_code is set to zero because there are no replies when sending data blocks
	{command_DATAEND,       3*30,  10*60, 250, ECSmtp::MSG_BODY_ERROR},
	{command_QUIT,          1*30,  1*30,  221, ECSmtp::COMMAND_QUIT},
	{command_STARTTLS,      1*30,  1*30,  220, ECSmtp::COMMAND_EHLO_STARTTLS}
};
...
void CSmtp::Send()
{
	...
	try{
		//Allocate memory
		//printf("====smtp connect=0000===\n");
		if((FileBuf = new char[FILE_READ_BUFF_SIZE+1]) == NULL)
			throw ECSmtp(ECSmtp::LACK_OF_MEMORY);
		//printf("====smtp connect=1111===\n");
		if((FileName = new char[255]) == NULL)
			throw ECSmtp(ECSmtp::LACK_OF_MEMORY);
#if 0//David.Milesight add 14.10.09
		//Check that any attachments specified can be opened
		TotalSize = 0;
		//printf("====smtp connect=2222===\n");
		...
		{	
...
#if 0 //David.Milesight add 14.10.09			
			while(!feof(hFile))
				FileSize += fread(FileBuf,sizeof(char),FILE_READ_BUFF_SIZE,hFile);
#else
			int in;
			...
			if( (fs_temp = (unsigned long int)lseek(in, 0, SEEK_END)) == -1 ) {
				printf("fseek SEEK_END error");
				close(in);
				return ;
			}
			FileSize = (unsigned long int)fs_temp;
			close(in);
#endif			
			TotalSize += FileSize;
			...
			else
			{
				...
				MsgPart = 0;
				for(i=0;i<FileSize/FILE_READ_BUFF_SIZE+1;i++)
				{
					res = fread(FileBuf,sizeof(char),FILE_READ_BUFF_SIZE,hFile);
		...
		//printf("====smtp connect=9999===\n");	
		pEntry = FindCommandEntry(command_DATAEND);
		// <CRLF> . <CRLF>
		//printf("====smtp connect=aaaa===\n");	
		snprintf(SendBuf, BUFFER_SIZE, "\r\n.\r\n");
		SendData(pEntry);
		//printf("====smtp connect=bbbb===\n");	
		ReceiveResponse(pEntry);
		//printf("====smtp connect=cccc===\n");	
	}
	catch(const ECSmtp&)
	{
        m_bQuitNoResponse = true;
								...
								const char* password/*=NULL*/)
{
	...
	try
	{
		...
		if(authenticate && IsKeywordSupported(RecvBuf, "AUTH") == true)
		{
            ...
            else if(IsKeywordSupported(RecvBuf, "NTLM") == true)
            {
                if (m_debug) {
                    printf("###### NTLM #####\n");
                }
...
void CSmtp::FormatHeader(char* header)
{
	...
	char week[][5] = {"Sun", "Mon", "Tue", "Wed", "Thur", "Fri", "Sat"};
	size_t i;
	std::string to;
	std::string cc;
	std::string bcc;
	...
	{
		timeinfo->tm_gmtoff = -timeinfo->tm_gmtoff;
		timezone[0] = '-';
	}
	else
	{
	...
	timezone_minute = timeinfo->tm_gmtoff/60;
	timezone_hour = timezone_minute/60;
	timezone_minute = timezone_minute%60;
	snprintf(timezone+1, sizeof(timezone)-1,"%02d%02d", timezone_hour, timezone_minute);
	// Date: <SP> <dd> <SP> <mon> <SP> <yy> <SP> <hh> ":" <mm> ":" <ss> <SP> <zone> <CRLF>
	snprintf(header, BUFFER_SIZE, "Date: %s, %d %s %d %02d:%02d:%02d %s\r\n",  week[timeinfo->tm_wday],	timeinfo->tm_mday,
																								month[timeinfo->tm_mon],
																								timeinfo->tm_year+1900,
																								timeinfo->tm_hour,
																								timeinfo->tm_min,
																								timeinfo->tm_sec, timezone); 
...
void CSmtp::ReceiveData(Command_Entry* pEntry)
{
	...
	//printf("=====smtp ReceiveData====7777===%d==\n", time.tv_sec);
...
void CSmtp::ReceiveResponse(Command_Entry* pEntry)
{
    ...
    if (m_debug) {
	    printf("=%d==%d==smtp connect====8888===%s==\n", reply_code, pEntry->valid_reply_code, RecvBuf);
    }
	OutputDebugStringA(RecvBuf);
	if(reply_code != pEntry->valid_reply_code)
	{
		FILE *fp = fopen("/tmp/smtp.msg", "wb");
		if(fp)
		{
			fprintf(fp, "reply code:%d != valid code:%d\nSendBuf:%s\nRecvBuf:%s\n", 
                reply_code, pEntry->valid_reply_code, SendBuf, RecvBuf);
			fflush(fp);
			fclose(fp);
		}		
