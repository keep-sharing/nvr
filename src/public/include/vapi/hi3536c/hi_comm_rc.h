...
typedef struct hiVENC_ATTR_H264_CVBR_S
{
    HI_U32  u32Gop;                     /* the interval of ISLICE. */
    HI_U32  u32StatTime;                /* the rate statistic time, the unit is senconds(s) */
    HI_U32  u32SrcFrmRate;              /* the input frame rate of the venc chnnel */
    ...
    HI_U32  u32ShortTermStatTime;       /* the long-term rate statistic time, the unit is second (s)*/
    HI_U32  u32LongTermStatTime;        /* the long-term rate statistic time, the unit is u32LongTermStatTimeUnit*/
    HI_U32  u32LongTermMaxBitrate;      /* the long-term target max bitrate, can not be larger than u32MaxBitRate,the unit is kbps */
    HI_U32  u32LongTermMinBitrate;      /* the long-term target min bitrate,  can not be larger than u32LongTermMaxBitrate,the unit is kbps */
...
typedef struct hiVENC_ATTR_H264_CVBR_S  VENC_ATTR_H265_CVBR_S;
...
typedef struct hiVENC_RC_ATTR_S
{
    ...
    union
    {
        ...
        VENC_ATTR_H264_CVBR_S   stAttrH264CVbr;
        ...
        VENC_ATTR_H265_CVBR_S   stAttrH265CVbr;
...
{
    HI_U32  u32MinIprop;                /* the min ratio of i frame and p frame */
    HI_U32  u32MaxIprop;                /* the max ratio of i frame and p frame,can not be smaller than u32MinIprop */
    HI_S32  s32MaxReEncodeTimes;        /* max number of re-encode times */
    HI_BOOL bQpMapEn;                   /* enable qpmap.*/
    HI_U32  u32MaxQp;                   /* the max P B qp */
    HI_U32  u32MinQp;                   /* the min P B qp,can not be larger than u32MaxQp */
    HI_U32  u32MaxIQp;                  /* the max I qp */
    HI_U32  u32MinIQp;                  /* the min I qp,can not be larger than u32MaxIQp */
    HI_U32  u32MinQpDelta;              /* Difference between FrameLevelMinQp and MinQp, FrameLevelMinQp = MinQp(or MinIQp) + MinQpDelta  */
    HI_U32  u32MaxQpDelta;              /* Difference between FrameLevelMaxQp and MaxQp, FrameLevelMaxQp = MaxQp(or MaxIQp) - MaxQpDelta  */
    HI_U32  u32ExtraBitPercent;         /* the extra bits that can be allocated when the actual bitrate goes above the target bitrate*/
    HI_U32  u32LongTermStatTimeUnit;    /* the time unit of LongTermStatTime,, the unit is senconds(s)*/
...
typedef struct hiVENC_PARAM_H265_CVBR_S
{
    ...
    HI_S32  s32MaxReEncodeTimes;        /* max number of re-encode times */
    HI_BOOL bQpMapEn;                   /* enable qpmap.*/
    VENC_RC_QPMAP_MODE_E enQpMapMode;   /* Qpmap Mode*/
    HI_U32  u32MaxQp;                   /* the max P B qp */
    HI_U32  u32MinQp;                   /* the min P B qp,can not be larger than u32MaxQp */
    HI_U32  u32MaxIQp;                  /* the max I qp */
    HI_U32  u32MinIQp;                  /* the min I qp,can not be larger than u32MaxIQp */
    HI_U32  u32MinQpDelta;              /* Difference between FrameLevelMinQp and MinQp, FrameLevelMinQp = MinQp(or MinIQp) + MinQpDelta  */
    HI_U32  u32MaxQpDelta;              /* Difference between FrameLevelMaxQp and MaxQp, FrameLevelMaxQp = MaxQp(or MaxIQp) - MaxQpDelta  */
    HI_U32  u32ExtraBitPercent;         /* the extra ratio of bitrate that can be allocated when the actual bitrate goes above the long-term target bitrate*/
    HI_U32  u32LongTermStatTimeUnit;    /* the time unit of LongTermStatTime, the unit is senconds(s)*/
