...
"table first (use 'o'). This will destroy the present disk contents.");
        ...
        /* c52662 */
	for (i = 0; i < 4; i++)
		free_primary += !ptes[i].part_table->sys_ind; /* c52662*/
	...
	if (!free_primary) {
		...
		while (1) {
			...
			if (c == 'l' && extended_offset) { /* c52662 */
				add_logical();
				return;
			}
			if (c == 'e' && !extended_offset) { /* c52662 */
				i = get_nonexisting_partition();
				if (i >= 0)
...
#if ENABLE_FEATURE_FDISK_WRITABLE
	if (opt & OPT_l) { /* c52662:fdisk -l */
		nowarn = 1;
#endif
		if (*argv) { /* c52662:"fdisk -l devname */
			listing = 1;
			do {
