...
#include "unicode.h"
#if ENABLE_FEATURE_RTOS_LOG_CMD
#include <syslog.h>
...
#else
//config: config FEATURE_RTOS_LOG_CMD
//config: bool "Provide busybox commandline log"
//config: default y
//config: depends on SHELL_ASH
//config: help
//config: With this option busybox support record the commandline
//config: log. It can use env RTOS_LOG_IP to control whether
//config: the log record the user's IP. The env RTOS_LOG_IP=yes 
//config: or RTOS_LOG_IP is not set means the cmd log would
//config: record the user's IP. The env RTOS_LOG_IP=no means
//config: doesn't record the user's IP. It forbids the shell
//config: change password command via pipe.
#if ENABLE_FEATURE_RTOS_LOG_CMD
uint8_t exitstatus_addr;
int show_ip = 1;
#define RTOS_PATH "/proc/self/fd/0"
...
static bool get_user_ip(struct utmpx **ut)
{
	char *tty_name = NULL;
	bool flag = false;
...
static void rtos_log_cmd(int retval)
{
	char buf[HISTLEN] = {0};
	ssize_t ret = 0;
