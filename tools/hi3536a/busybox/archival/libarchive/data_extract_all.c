...
//TODO: what if file_header->link_target == NULL (say, corrupted tarball?)
	...
	// when ENABLE_FEATURE_RTOS_TAR_RESTORE_PERMS is on, do not skip link
	if (ENABLE_FEATURE_RTOS_TAR_RESTORE_PERMS || !S_ISLNK(file_header->mode)) {
...
#endif
#if !ENABLE_FEATURE_RTOS_TAR_RESTORE_PERMS
			/* GNU tar 1.15.1 uses chown, not lchown */
			chown(dst_name, uid, gid);
#else
			// Also deal with link when ENABLE_FEATURE_RTOS_TAR_RESTORE_PERMS is on
